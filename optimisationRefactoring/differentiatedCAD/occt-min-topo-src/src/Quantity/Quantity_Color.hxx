...
public:
  ...
  Standard_EXPORT double Distance (const Quantity_Color& AColor) const;
  
  //! Returns the square of distance between two colours.
  Standard_EXPORT double SquareDistance (const Quantity_Color& AColor) const;
...
private:
  ...
  //! Converts HLS components into RGB ones.
  Standard_EXPORT static void hlsrgb (const float H, const float L, const float S, float& R, float& G, float& B);
  
  //! Converts RGB components into HLS ones.
  Standard_EXPORT static void rgbhls (const float R, const float G, const float B, float& H, float& L, float& S);
  ...
  //! <R2> is the lightness between 0. and 1.
  //! <R3> is the saturation between 0. and 1.
  Standard_EXPORT static void ValuesOf (const Quantity_NameOfColor AName, const Quantity_TypeOfColor AType, float& R1, float& R2, float& R3);
