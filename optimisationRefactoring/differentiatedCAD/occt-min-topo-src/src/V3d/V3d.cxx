...
void V3d::DrawSphere(const Handle(V3d_Viewer)& aViewer,const Standard_Real ray)
{
  const Standard_Boolean inf = ray < 0;
  const Standard_Real aRadius = Standard_ShortReal(Abs(ray).getValue());
  ...
  for( j=0; j<NFACES/2 ; j++, Beta += Dbeta ) {
    ...
    R = aRadius*sin(Beta);
    Z = Standard_ShortReal((aRadius*cos(Beta)).getValue());
    for( i=0, Alpha = 0.; i<NFACES; i++, Alpha += Dalpha ) {
      X = Standard_ShortReal((R*cos(Alpha)).getValue());
      Y = Standard_ShortReal((R*sin(Alpha)).getValue());
      aPrims->AddVertex(X,Y,Z);
  ...
  for( j=0; j<NFACES/2 ; j++, Beta += Dbeta ) {
    aPrims->AddBound(NFACES+1);
    R = aRadius*sin(Beta);
    Y = Standard_ShortReal((aRadius*cos(Beta)).getValue());
    Beta += Dbeta ;
    for( i=0, Alpha = 0.; i<NFACES; i++, Alpha += Dalpha ) {
      X = Standard_ShortReal((R*cos(Alpha)).getValue());
      Z = Standard_ShortReal((R*sin(Alpha)).getValue());
      aPrims->AddVertex(X,Y,Z);
      if (i==0) { X0=X, Y0=Y, Z0=Z; }
    }
    aPrims->AddVertex(X0,Y0,Z0);
