...
void V3d_SpotLight::SetPosition (const Standard_Real theXp,
                                 ...
                                 const Standard_Real theZp)
{
  myLight.Position.x() = static_cast<Standard_ShortReal> (theXp.getValue());
  myLight.Position.y() = static_cast<Standard_ShortReal> (theYp.getValue());
  myLight.Position.z() = static_cast<Standard_ShortReal> (theZp.getValue());
}
...
{
  myLight.Direction.x() = static_cast<Standard_ShortReal> (theVx.getValue());
  myLight.Direction.y() = static_cast<Standard_ShortReal> (theVy.getValue());
  myLight.Direction.z() = static_cast<Standard_ShortReal> (theVz.getValue());
}
                                    ...
                                    const Standard_Real theLinearAttenuation)
{
  ...
  myLight.ChangeConstAttenuation()  = static_cast<Standard_ShortReal> (theConstAttenuation.getValue());
  myLight.ChangeLinearAttenuation() = static_cast<Standard_ShortReal> (theLinearAttenuation.getValue());
}
...
void V3d_SpotLight::SetConcentration (const Standard_Real theConcentration)
{
  ...
  myLight.ChangeConcentration() = static_cast<Standard_ShortReal> (theConcentration.getValue());
}
...
void V3d_SpotLight::SetAngle (const Standard_Real theAngle)
{
  ...
  myLight.ChangeAngle() = static_cast<Standard_ShortReal> (theAngle.getValue());
}
...
//Display of the position of the light.
  ...
  Quantity_Color Col1(R1.getValue(),G1.getValue(),B1.getValue(),Quantity_TOC_RGB);
...
// Display of the radius of the sphere (line + text)
    ...
    if (Pres == V3d_COMPLETE) {
      ...
      TCollection_AsciiString ValOfRadius(Rayon.getValue());
