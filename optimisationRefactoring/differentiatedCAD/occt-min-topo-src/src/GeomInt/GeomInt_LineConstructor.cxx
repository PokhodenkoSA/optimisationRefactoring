...
void GeomInt_LineConstructor::Perform(const Handle(IntPatch_Line)& L)
{
  ...
  if(typl == IntPatch_Analytic)  {
    ...
    for(i=1;i<nbvtx;i++)    { 
      ...
      if(firstp!=lastp) { 
        if(lastp != firstp+1)  {
          const Standard_Integer pmid = (Standard_Integer )( (firstp+lastp)/2).getValue();
          ...
          const IntSurf_PntOn2S& Pfirst = WLine->Point((Standard_Integer)(firstp.getValue()));
          ...
          if(in1 !=  TopAbs_OUT) {  //-- !=ON donne Pb 
            ...
            if(in2 != TopAbs_OUT) { //-- !=ON  
              const IntSurf_PntOn2S& Plast = WLine->Point((Standard_Integer)(lastp.getValue()));
    ...
    if (aNbParts > 1) {  
      ...
      if (bCond) {
        ...
        for(i=1; i<=aNb; ++i) {
          lastp =seqp(i);
          anIndex=(Standard_Integer)lastp.getValue();
                             ...
                             const Standard_Real aPeriod)
{
  ...
  aT=aTr;
  if (aT<0.) {
    k=-(Standard_Integer)(aT/aPeriod).getValue()+1;
    aT=aT+k*aPeriod;
  }
  //
  if (!(aT>=0. && aT<=aPeriod)) {
    k=(Standard_Integer)(aT/aPeriod).getValue();
    aT=aT-k*aPeriod;
  }
  //
  return aT;
