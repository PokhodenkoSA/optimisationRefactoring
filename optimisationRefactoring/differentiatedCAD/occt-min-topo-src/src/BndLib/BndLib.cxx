...
#define addPoint7 theB.Add(PointType(theO.Coord() +aRam*PI4*theXd.Coord() -aRbm*PI4*theYd.Coord()))
    ...
    Standard_Integer aDeb = (Standard_Integer )( aTeta1.getValue()/(M_PI/4.));
    Standard_Integer aFin = (Standard_Integer )( aTeta2.getValue()/(M_PI/4.));
    aDeb++;
...
void BndLib::Add(const gp_Torus& S,const Standard_Real UMin,
  ...
  if (VMax<VMin) {
    Fi1 = (Standard_Integer )( VMax.getValue()/(M_PI/4.));
    Fi2 = (Standard_Integer )( VMin.getValue()/(M_PI/4.));
  }
  else {
    Fi1 = (Standard_Integer )( VMin.getValue()/(M_PI/4.));
    Fi2 = (Standard_Integer )( VMax.getValue()/(M_PI/4.));
  }
  Fi2++;
