			 ...
			 const Standard_Boolean compact)
{
  ...
  Print(C2d.YAxis().Direction(),OS,compact);
  if (!compact) OS << "\n  Radii  :";
  OS << C2d.MajorRadius().getValue();
  if (!compact) OS << ",";
  OS << " ";
  OS << C2d.MinorRadius().getValue();
  if (!compact) OS << "\n";
  OS << "\n";
}
			 ...
			 const Standard_Boolean compact)
{
  ...
  for (i = 1; i <= nbknots; i++) {
    ...
    OS << B->Knot(i).getValue() << " " << B->Multiplicity(i);
  }
  if (!compact) OS << "\n";
  OS << "\n";
}
			 ...
			 const Standard_Boolean compact)
{
  ...
  OS << C->FirstParameter().getValue() << " " << C->LastParameter().getValue() << "\n";
