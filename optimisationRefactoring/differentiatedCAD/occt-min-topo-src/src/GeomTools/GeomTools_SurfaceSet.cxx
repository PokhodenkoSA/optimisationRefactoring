			 ...
			 const Standard_Boolean compact)
{
  ...
  Print(P.YAxis().Direction(),OS,compact);
  if (!compact) OS << "\n  Radius :";
  OS << P.RefRadius().getValue();
  OS << "\n";
  if (!compact) OS << "\n  Angle :";
  OS << P.SemiAngle().getValue();
  OS << "\n";
  if (!compact) OS << "\n";
}
			 ...
			 const Standard_Boolean compact)
{
  ...
  Print(P.YAxis().Direction(),OS,compact);
  if (!compact) OS << "\n  Radii  :";
  OS << P.MajorRadius().getValue() << " " << P.MinorRadius().getValue();
  OS << "\n";
  if (!compact) OS << "\n";
}
			 ...
			 const Standard_Boolean compact)
{
  ...
  for (i = 1; i <= nbuknots; i++) {
    if (!compact) OS << "\n  "<<setw(2)<<i<<" : ";
    OS << S->UKnot(i).getValue() << " " << S->UMultiplicity(i) <<"\n";
  }
  OS << "\n";
  if (!compact) OS << "\n VKnots :\n";
  for (i = 1; i <= nbvknots; i++) {
    if (!compact) OS << "\n  "<<setw(2)<<i<<" : ";
    OS << S->VKnot(i).getValue() << " " << S->VMultiplicity(i) <<"\n";
  }
  OS << "\n";
  if (!compact) OS << "\n";
}
			 ...
			 const Standard_Boolean compact)
{
  ...
  if (!compact) OS << "\nParameters : ";
  OS << U1.getValue() << " " << U2.getValue() << " " << V1.getValue() << " " << V2.getValue() <<"\n";
