...
void GeomPlate_BuildPlateSurface::ComputeSurfInit()
{
  ...
  if (nopt != 3)
	...
	LenT+=myLinCont->Value(i)->Length();
      for (i=1;i<=NTLinCont;i++) 
	{ Standard_Integer NbPoint= (Standard_Integer )( NTPoint*(myLinCont->Value(i)->Length())/LenT).getValue();
	  if (NbPoint<10)
	    NbPoint=10;
	  Npt+=NbPoint;
	}
  ...
  for ( i=1;i<=NTLinCont;i++) 
	...
	{ case 0 : // Cas G0 *1.2
	    myLinCont->ChangeValue(i)->SetNbPoints( 
						   Standard_Integer((1.2*NTPoint*(myLinCont->Value(i)->Length())/LenT).getValue()));
	    break;
	  case 1 : // Cas G1 *1
	    myLinCont->ChangeValue(i)->SetNbPoints(
				 Standard_Integer((NTPoint*(myLinCont->Value(i)->Length())/LenT).getValue()));
	    break;
	  case 2 : // Cas G2 *0.7
	    myLinCont->ChangeValue(i)->SetNbPoints( 
			      Standard_Integer((0.7*NTPoint*(myLinCont->Value(i)->Length())/LenT).getValue()));
	    break;
	  } 
...
VerifSurface(const Standard_Integer NbBoucle)
{
  ...
  for (Standard_Integer i=1;i<=NTLinCont;i++) {
	...
	if (NdiffDist>0) {// au moins un point n'est pas acceptable en G0
	  ...
	  LinCont->SetNbPoints(Standard_Integer((LinCont->NbPoints() * Coef).getValue()));
	  Result=Standard_False;	    	
	}
     else
