...
Handle(IntPatch_RLine) BuildRLine(const IntPatch_SequenceOfLine&     theSeqOfWLine, 
  ...
  for(theDomain->Init(); theDomain->More(); theDomain->Next()) {
	...
	for(Standard_Integer j = 1; j<=theSeqOfWLine.Length(); j++) {
	  ...
	  Standard_Integer ParamMinOnLine = (Standard_Integer) atmpWLine->Vertex(1).ParameterOnLine().getValue();
	  Standard_Integer ParamMaxOnLine = (Standard_Integer) atmpWLine->Vertex(atmpWLine->NbVertex()).ParameterOnLine().getValue();
