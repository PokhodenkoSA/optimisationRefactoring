...
void BRepExtrema_DistShapeShape::Dump(Standard_OStream& o) const 
{
  ...
  for (i=1;i<=NbSolution();i++)
  {
    ...
    o<<"X=" <<PointOnShape1(i).X().getValue()<<" Y=" <<PointOnShape1(i).Y().getValue()<<" Z="<<PointOnShape1(i).Z().getValue()<<endl;
    o<< "the coordinates of  the point on the second shape are: "<<endl; 
    o<<"X="<< PointOnShape2(i).X().getValue()<< " Y="<<PointOnShape2(i).Y().getValue()<<" Z="<< PointOnShape2(i).Z().getValue()<<endl;
    ...
    switch (SupportTypeShape1(i))
    {
      ...
      case BRepExtrema_IsOnEdge:
        ParOnEdgeS1(i,r1);
        o << "parameter on the first edge :  t= " << r1.getValue() << endl;
        break;
      case BRepExtrema_IsInFace:
        ParOnFaceS1(i,r1,r2);
        o << "parameters on the first face :  u= " << r1.getValue() << " v=" <<  r2.getValue() << endl;
        break;
    }
    switch (SupportTypeShape2(i))
    {
      ...
      case BRepExtrema_IsOnEdge:
        ParOnEdgeS2(i,r1);
        o << "parameter on the second edge : t=" << r1.getValue() << endl;
        break;
      case BRepExtrema_IsInFace:
        ParOnFaceS2(i,r1,r2);
        o << "parameters on the second face : u= " << r1.getValue() << " v=" <<  r2.getValue() << endl;
        break;
    }
    o<<endl;
  } 
