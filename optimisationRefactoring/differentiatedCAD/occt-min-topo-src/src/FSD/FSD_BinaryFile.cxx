...
Storage_BaseDriver& FSD_BinaryFile::PutReal(const double aValue)
{
  ...
  if (!fwrite(&t,sizeof(double),1,myStream)) Storage_StreamWriteError::Raise();
#else
  if (!fwrite(&aValue,sizeof(double),1,myStream)) Storage_StreamWriteError::Raise();
#endif
  return *this;
}
...
Storage_BaseDriver& FSD_BinaryFile::PutShortReal(const float aValue)
{
  ...
  if (!fwrite(&t,sizeof(float),1,myStream)) Storage_StreamWriteError::Raise();
#else
  if (!fwrite(&aValue,sizeof(float),1,myStream)) Storage_StreamWriteError::Raise();
#endif
  return *this;
}
...
{
  Standard_STATIC_ASSERT(sizeof(float) == sizeof(Standard_Integer));
  union {
    float aValue;
    Standard_Integer   aResult;
  } aWrapUnion;
