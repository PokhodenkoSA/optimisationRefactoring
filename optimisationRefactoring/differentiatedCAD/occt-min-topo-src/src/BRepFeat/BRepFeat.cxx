                          ...
                          Handle(Geom2d_Curve)&       C2d)
{
  ...
  Standard_Real du = 0.;
  if (minC< umin - eps) {
    du = (int(((umin - minC)/period).getValue()) + 1)*period;
  }
  if (minC > umax + eps) {
    du = -(int(((minC - umax)/period).getValue()) + 1)*period;
  }
  if (du != 0) {
    gp_Vec2d T1(du,0.);
    C2d->Translate(T1);
                          ...
                          Handle(Geom2d_Curve)&       C2d)
{
  ...
  Standard_Real dv = 0.;
  if (minC< vmin - eps) {
    dv = (int(((vmin - minC)/period).getValue()) + 1)*period;
  }
  if (minC > vmax + eps) {
    dv = -(int(((minC - vmax)/period).getValue()) + 1)*period;
  }
  if (dv != 0) {
    gp_Vec2d T1(0.,dv);
    C2d->Translate(T1);
