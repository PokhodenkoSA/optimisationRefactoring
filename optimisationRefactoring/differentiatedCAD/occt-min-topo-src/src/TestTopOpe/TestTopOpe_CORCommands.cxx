...
{di <<" = ("<<p2d.X().getValue()<<" "<<p2d.Y().getValue()<<") ";}
...
static Standard_Integer pntonc(Draw_Interpretor& di, Standard_Integer n, const char** a)
{
  ...
  gp_Pnt p = C->Value(x);
  di<<"point on curve of parameter "<<x.getValue()<<" =("<<p.X().getValue()<<",";
  di<<p.Y().getValue()<<","<<p.Z().getValue()<<")"<<"\n";
  return 0;
}
...
static Standard_Integer pntonc2d(Draw_Interpretor& di, Standard_Integer n, const char** a)
{
  ...
  di<<"point on curve of parameter "<<x.getValue()<<" =("<<p2d.X().getValue()<<","<<p2d.Y().getValue()<<")"<<"\n";
...
static Standard_Integer projponf(Draw_Interpretor& di, Standard_Integer n, const char** a)
{
  ...
  //
  di<<"proj dist = "<<dist.getValue()<<" uvproj = ("<<uv.X().getValue()<<" "<<uv.Y().getValue();
  di<<"); pproj = ("<<pproj.X().getValue()<<" "<<pproj.Y().getValue()<<" "<<pproj.Z().getValue()<<")"<<"\n";
  return 0;
}  
...
//  TopoDS_Face f = TopoDS::Face(DBRep::Get(a[1]));
  ...
  for (Standard_Integer i = 1; i <= npnts; i++) {
    ...
    di<<"point("<<i<<") = { ("<<p.X().getValue()<<" "<<p.Y().getValue()<<" "<<p.Z().getValue()<<"), ("<<u.getValue()<<" "<<v.getValue()<<") }"<<"\n";
    ...
    Norm = gp_Dir(D1^D2); 
    di<<"D1  = ("<<D1.X().getValue()<<" "<<D1.Y().getValue()<<" "<<D1.Z().getValue()<<" "<<")"<<"\n";
    di<<"D2  = ("<<D2.X().getValue()<<" "<<D2.Y().getValue()<<" "<<D2.Z().getValue()<<" "<<")"<<"\n";
    di<<"Norm  = ("<<Norm.X().getValue()<<" "<<Norm.Y().getValue()<<" "<<Norm.Z().getValue()<<" "<<")"<<"\n";
  } // i
  return 0;
}
