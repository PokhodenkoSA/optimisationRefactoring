			      ...
			      const double moveY)
{
  ...
  case DRAW :
    {
      int X =   (int) ( pt.X() + moveX + curview->GetDx()).getValue();
      int Y =   (int) (-pt.Y() + moveY - curview->GetDy()).getValue();
  ...
  case POSTSCRIPT :
    {
      Standard_Integer x = (Standard_Integer )( (pt.X() + moveX - ps_vx) * ps_kx + ps_px).getValue();
      Standard_Integer y = (Standard_Integer )( (pt.Y() + moveY - ps_vy) * ps_ky + ps_py).getValue();
      (*ps_stream) <<"stroke\n";
      (*ps_stream) << x << " " << y << " m\n";
      (*ps_stream) <<"("<<S<<") show\nnewpath\n";
    }
...
void Draw_Display::MoveTo (const gp_Pnt2d& pp)
{
  ...
  case POSTSCRIPT :
    {
      Standard_Integer x = (Standard_Integer )( (pt.X() - ps_vx) * ps_kx + ps_px).getValue();
      Standard_Integer y = (Standard_Integer )( (pt.Y() - ps_vy) * ps_ky + ps_py).getValue();
      (*ps_stream) <<"stroke\nnewpath\n"<< x << " " << y << " m\n";
    }
    break;
  }
...
void Draw_Display::DrawTo (const gp_Pnt2d& pp2)
{
  ...
  case DRAW : {
    ...
    {
      segm[nbseg].Init(static_cast<Standard_Integer>( (PI1.X() + curview->GetDx()).getValue()),
                       static_cast<Standard_Integer>((-PI1.Y() - curview->GetDy()).getValue()),
                       static_cast<Standard_Integer>( (PI2.X() + curview->GetDx()).getValue()),
                       static_cast<Standard_Integer>((-PI2.Y() - curview->GetDy()).getValue()));
      ++nbseg;
    }
#else
  ...
  case PICK :
    if (!found) {
      Standard_Integer x1 =  (int) p1.X().getValue() ;
      Standard_Integer y1 =  (int) p1.Y().getValue() ;
      Standard_Integer x2 =  (int) p2.X().getValue() ;
      Standard_Integer y2 =  (int) p2.Y().getValue() ;
  ...
  case POSTSCRIPT :
    {
      Standard_Integer x = (Standard_Integer )( (p2.X() - ps_vx) * ps_kx + ps_px).getValue();
      Standard_Integer y = (Standard_Integer )( (p2.Y() - ps_vy) * ps_ky + ps_py).getValue();
      (*ps_stream) << x << " " << y << " l\n";
    }
    break;
  }
