   ...
   const Handle(Message_Messenger)& S, const Standard_Integer level)  const
{
  ...
  IGESData_DumpValsAD(S ,level,1, nbUSegs+1,ent->UBreakPoint);
  S <<endl<< "The V Break Points : ";
  IGESData_DumpValsAD(S ,level,1, nbVSegs+1,ent->VBreakPoint);
  ...
  if (level > 4)
	...
	for (J = 1; J <= nbVSegs; J++) 
	    ...
	    IGESData_DumpValsAD(S,level,1, temp->Length(),temp->Value);
	    S << endl;
	    S << "Y Polynomial : ";
