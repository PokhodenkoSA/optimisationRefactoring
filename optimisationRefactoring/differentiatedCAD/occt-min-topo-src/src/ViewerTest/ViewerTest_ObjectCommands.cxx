                      ...
                      const char**      theArgVec)
{
  ...
  for (; anArgIt < theArgsNb; ++anArgIt)
  {
    ...
    else if (aParam == "-color")
    {
      ...
      aTextPrs->SetColor (Quantity_Color (anRGB.r().getValue(), anRGB.g().getValue(), anRGB.b().getValue(), Quantity_TOC_RGB));
      anArgIt += 3;
    }
          ...
          || aParam == "-subtitlecolor")
    {
      ...
      aTextPrs->SetColorSubTitle (Quantity_Color (anRGB.r().getValue(), anRGB.g().getValue(), anRGB.b().getValue(), Quantity_TOC_RGB));
      anArgIt += 3;
    }
    else if (aParam == "-2d")
    {
...
Handle( Poly_Triangulation ) CalculationOfSphere( double X , double Y , double Z ,
  ...
  Handle(TColStd_HArray1OfReal) Normals = new TColStd_HArray1OfReal(1, polyTriangulation->NbNodes() * 3);
                       ...
                       const char**      theArgVec)
{
  ...
  if (!aFont.Init (aFontName.ToCString(), aFontAspect, aTextHeight.getValue()))
  {
  ...
  if (aCmd == CloudForShape)
  {
    ...
    for (TopExp_Explorer aFaceIt (aShape, TopAbs_FACE); aFaceIt.More(); aFaceIt.Next())
    {
      ...
      for (Standard_Integer aNodeIter = aNodes.Lower(); aNodeIter <= aNodes.Upper(); ++aNodeIter)
      {
        ...
        if (toRandColors)
        {
          Quantity_Color aColor ((360.0 * Standard_Real(anIndexOfPoint) / Standard_Real(aNbPoints)).getValue(),
