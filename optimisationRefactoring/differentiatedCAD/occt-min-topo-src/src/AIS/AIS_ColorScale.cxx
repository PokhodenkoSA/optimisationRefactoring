...
TCollection_ExtendedString AIS_ColorScale::GetLabel (const Standard_Integer theIndex) const
{
  ...
  sprintf (aBuf, aFormat.ToCString(), aVal.getValue());
...
Standard_Real AIS_ColorScale::GetLogNumber (const Standard_Integer theIndex) const
{
  if (GetNumberOfIntervals() > 0)
  {
    ...
    Standard_Real aDivisor = Pow (myMax/aMin, 1.0/myInterval);
                                            ...
                                            Quantity_Color& theColor)
{
  ...
  else
  {
    ...
    Standard_Integer anInterv = Standard_Integer (anIntervNumber.getValue());
                             ...
                             const Standard_Integer /*theMode*/)
{
  ...
  double anIPart;
  Standard_Real anFPart = modf (aVal.getValue(), &anIPart);
  ...
  {
    Standard_Integer aY = (Standard_Integer)( myYPos + ( i - 1 )* aStep + anOffset ).getValue();
    Standard_Integer aColorHeight = (Standard_Integer)( myYPos + ( i ) * aStep + anOffset ).getValue() - aY;
  ...
  if (aStep > 0)
    DrawFrame (thePresentation, aX - 1, (Standard_Integer)(myYPos + anOffset - 1).getValue(), aColorWidth + 2, (Standard_Integer)(aColors.Length() * aStep + 2).getValue(), aFgColor);
  ...
  if (toDrawLabel && aLabels.Length() && aFilter > 0)
  {
    ...
    while (i2 - i1 >= aFilter || ( i2 == 0 && i1 == 0 ))
    {
      ...
      {
        DrawText (thePresentation, aLabels.Value (i1 + 1), aX, (Standard_Integer)( myYPos + i1 * aStep + anAscent + anOffset ).getValue(), aFgColor);
        aLast1 = i1;
      }
      if (aFilter && !( aPos2 % aFilter ))
      {
        DrawText (thePresentation, aLabels.Value (i2 + 1), aX, (Standard_Integer)( myYPos + i2 * aStep + anAscent + anOffset ).getValue(), aFgColor);
        aLast2 = i2;
      }
      i1++;
      i2--;
    ...
    if (i0 != -1)
      DrawText (thePresentation, aLabels.Value (i0 + 1), aX, (Standard_Integer)( myYPos + i0 * aStep + anAscent + anOffset ).getValue(), aFgColor);
  }
}
