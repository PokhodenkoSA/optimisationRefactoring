...
//modified by NIZNHY-PKV Mon Apr 24 14:43:24 2000t
  ...
  for (i=2; i<=aNbEdges; i++) {
    ...
    Standard_Boolean nonPU = (Abs(U) < 1e-7) ? Standard_True : Standard_False;
    Standard_Boolean nonPV = (Abs(V) < 1e-7) ? Standard_True : Standard_False;

    if(!nonPU && UP) {
      Standard_Real dU = fmod(Abs(U).getValue(), 2*M_PI);
      nonPU = (dU > 1e-7 && (2*M_PI - dU > 1e-7)) ? Standard_True : Standard_False;
    }
    
    if(!nonPV && VP) {
      Standard_Real dV = fmod(Abs(V).getValue(), 2*M_PI);
      nonPV = (dV > 1e-7 && (2*M_PI - dV > 1e-7)) ? Standard_True : Standard_False;
    }

//    printf("(fmod(Abs(U), 2*M_PI) =%lf\n", (fmod(Abs(U), 2*M_PI)));
//    printf(" (fmod(Abs(V), 2*M_PI) > 1e-7)=%lf\n", (fmod(Abs(V), 2*M_PI)));
...
{ 
  ...
  a=.5*(OuterU1+OuterU2);
  b=Standard_Integer((-a/TwoPI).getValue());
  k=(a<0.)? 1 : 0;
  x=(b+k)*TwoPI;
  //
