                                    ...
                                    Handle(IntPatch_WLine)& theWLline)
{
  ...
  for( ciV = 1; ciV <= cnbV; ciV++ )
  {
    ...
    if( VDMin != 0 )
    {
      ...
      const Handle(IntPatch_WLine) aWLine = Handle(IntPatch_WLine)::DownCast(aSLine);
      Standard_Integer tiVpar = (Standard_Integer)aWLine->Vertex(VDMin).ParameterOnLine().getValue();
      Standard_Integer ciVpar = (Standard_Integer)theWLline->Vertex(ciV).ParameterOnLine().getValue();
      ...
      for( iPo = 1; iPo <= cnbV; iPo++ )
      {
        ...
        Standard_Integer IPo = (Standard_Integer) Po.getValue();
        VPold.Append(IPo);
      }
...
{ 
  ...
  for(i=0;i<SU1;i++) { 
    for(j=0;j<SV1;j++) { 
      ...
      int ix = (int)((P.X()-x0  + dx2 )/dx).getValue();
      if(DansGrille(ix)) { 
        int iy = (int)((P.Y()-y0 + dy2)/dy).getValue();
        if(DansGrille(iy)) {
          int iz = (int)((P.Z()-z0 + dz2)/dz).getValue();
          if(DansGrille(iz)) {
