			    ...
			    Standard_Real& U, Standard_Real& V)
{
  ...
  Standard_Integer Irang = Standard_Integer(par.getValue());
  if (Irang == Nbptlin) {
    Irang--;
    par = 1.0;
  }
...
{
  Standard_Real par = IntegerPart(param);
  Standard_Integer irang = Standard_Integer(par.getValue()) + 1;
...
{
  Standard_Integer irang1 = Standard_Integer(IntegerPart(parmin).getValue()) + 2;
  Standard_Integer irang2 = Standard_Integer(IntegerPart(parmax).getValue()) + 1;
  gp_Pnt2d p1,p2;
  ...
  while (Domain->More()) {
    ...
    { 
      ...
      { 
		...
		NbEchant = (nbs < 2.0 ? 2 : (Standard_Integer)nbs.getValue());
	    if(NbEchant<10) NbEchant=10;
        else if (NbEchant>50) NbEchant=50;
      }
      break;
	...
	for (i=1;i<=Commun_Section_Tangent;i++) {
	  ...
	  if(i<=Commun_NbSectionPoints) { 
	    ...
	    Standard_Boolean isNptLow = (nptCh < 10. && nptCh < Nbptlin/100.) ||
	      (!Domain->Has3d() && Standard_Integer(nptCh.getValue())+1 < Nbptlin);
	    if (!isNptLow && !IsSegment2dSmall(Brise,UMinAr,UMaxAr,tolU,tolV)) {
	      ...
	      irangAr1 = Standard_Integer(UMinArP.getValue())+1;
              irangAr2 = Standard_Integer(UMaxArP.getValue())+1;
...
//	      Standard_Boolean isChExtr2 = irangCh2==Nbptlin;
	  ...
	  for (Standard_Integer ip=0; ip < nbpt; ip++) {
		...
		if (ptsommet.Distance(locpt(j)) <= edgeTol) {
	      ...
	      if (!duplicate) {
                Standard_Integer ParamApproxOnLine = Standard_Integer(W1[ip].getValue())+1;
		...
		if (typL == IntPatch_Walking)
		  VerifyTgline(wlin,(Standard_Integer)paramline.getValue(),edgeTol,tgline);
