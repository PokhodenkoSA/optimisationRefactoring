...
static int BUC60609(Draw_Interpretor& di, Standard_Integer argc, const char ** argv) {
  ...
  di << faceUMin.getValue() << " <= U <= " << faceUMax.getValue() << "\n";
  di << faceVMin.getValue() << " <= V <= " << faceVMax.getValue() << "\n";
...
//  change the object color with RGB values. 
    ...
    TopoDS_Shape aShape = DBRep::Get(argv[1]);
    Handle(AIS_InteractiveObject) myShape =  new AIS_Shape (aShape);
    myAISContext->SetColor(myShape,Quantity_Color(QuantityOfRed.getValue(),QuantityOfGreen.getValue(),QuantityOfBlue.getValue(),Quantity_TOC_RGB),Standard_True);
    myAISContext->Display(myShape,Standard_True);
    myAISContext->UpdateCurrentViewer();
