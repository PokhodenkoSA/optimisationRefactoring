...
static Standard_Integer CheckProps (Draw_Interpretor& di, Standard_Integer argc, const char** argv)
{
  ...
  for ( Standard_Integer i=1; i <= seq.Length(); i++ ) {
	...
	Sprintf (string2, "%s%9.1f (%3d%%)%s", ( wholeDoc ? "" : "  Area defect:   " ),
		 ...
		 (Standard_Integer)( Abs ( G.Mass() ) > 1e-10 ? (100. * ( aArea->Get() - G.Mass() ) / G.Mass()).getValue() : 999. ),
		 ( wholeDoc ? "" : "\n" ));
	di << string2;
      }
	...
	if ( ! aVolume.IsNull() ) {
	  ...
	  Sprintf (string5, "%s%9.1f (%3d%%)%s", ( wholeDoc ? "" : "  Volume defect: " ),
		   ...
		   (Standard_Integer)( Abs ( localVolume ) > 1e-10 ? (100. * ( aVolume->Get() - localVolume ) / localVolume).getValue() : 999. ),
		   ( wholeDoc ? "" : "\n" ));
	  di << string5;
	}
	else if ( wholeDoc ) {
