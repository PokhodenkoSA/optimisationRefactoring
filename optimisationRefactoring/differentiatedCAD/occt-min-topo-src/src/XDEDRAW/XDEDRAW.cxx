...
static Standard_Integer XAttributeValue (Draw_Interpretor& di, Standard_Integer argc, const char** argv)
{
  ...
  if ( att->IsKind(STANDARD_TYPE(TDataStd_TreeNode)) ) {
    ...
    di << "(" ;
    di << myCentroid.X().getValue();
    di <<" , ";
    di << myCentroid.Y().getValue();
    di <<" , ";
    di << myCentroid.Z().getValue();
    di << ")";
  }
    ...
    if(kind<20) { //dimension
      di<<"Diameter (ValueRange["<<HAR->Value(1).getValue()<<","<<HAR->Value(2).getValue()<<"])";
    }
    else {
      switch(kind) {
      case 21: di << "GeoTolAndGeoTolWthDatRefAndModGeoTolAndPosTol_1 (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 22: di << "GeoTolAndGeoTolWthDatRefAndModGeoTolAndPosTol_2 (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 23: di << "GeoTolAndGeoTolWthDatRefAndModGeoTolAndPosTol_3 (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 24: di << "AngularityTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 25: di << "CircularRunoutTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 26: di << "CoaxialityTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 27: di << "ConcentricityTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 28: di << "ParallelismTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 29: di << "PerpendicularityTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 30: di << "SymmetryTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 31: di << "TotalRunoutTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 35: di << "ModifiedGeometricTolerance_1 (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 36: di << "ModifiedGeometricTolerance_2 (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 37: di << "ModifiedGeometricTolerance_3 (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 38: di << "CylindricityTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 39: di << "FlatnessTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 40: di << "LineProfileTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 41: di << "PositionTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 42: di << "RoundnessTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 43: di << "StraightnessTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      case 44: di << "SurfaceProfileTolerance (Value="<<HAR->Value(1).getValue()<<")"; break;
      }
    }
  }
    ...
    if(dens==0) 
      ...
      di<<val->GetName()->ToCString()<<"(density="<<dens.getValue()<<dimdens<<")";
  }
