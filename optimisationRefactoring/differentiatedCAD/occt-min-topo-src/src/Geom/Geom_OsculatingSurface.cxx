  ...
  Handle(Geom_BSplineSurface)& BSpl) const
{
  ...
  {
    MinDegree = (Standard_Integer ) Min(udeg,vdeg).getValue() ;
    MaxDegree = (Standard_Integer ) Max(udeg,vdeg).getValue() ;
    ...
    Handle(TColStd_HArray1OfReal) TrueVIntervals = 
      new TColStd_HArray1OfReal(1, 2);
    MaxUDegree = (Standard_Integer ) udeg.getValue();
    MaxVDegree = (Standard_Integer ) vdeg.getValue();
    ...
    if (IsAlongU()) 
    {
      ...
      cout<<">>>>>>>>>>> AlongU"<<endl;
#endif
      OscUNumCoeff = (Standard_Integer ) udeg.getValue() + 1;
      OscVNumCoeff = (Standard_Integer ) vdeg.getValue();
    }
    if (IsAlongV()) 
    {
#ifdef OCCT_DEBUG
      cout<<">>>>>>>>>>> AlongV"<<endl;
#endif
      OscUNumCoeff = (Standard_Integer ) udeg.getValue();
      OscVNumCoeff = (Standard_Integer ) vdeg.getValue() + 1;
    }
