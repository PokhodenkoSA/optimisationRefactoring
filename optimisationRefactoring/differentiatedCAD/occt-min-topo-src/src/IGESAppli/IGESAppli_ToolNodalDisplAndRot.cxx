...
//     IGESData_DumpListXYZL(S,-level,1,nbcases,ent->RotationalParameter,loca);
      ...
      for (Standard_Integer j = 1; j <= nbnodes; j ++)
	{
	  ...
	  for (Standard_Integer k = 1; k <= nbcases; k ++)
		...
		<< ent->TranslationParameter(j,k).X().getValue() << ", Y="
		<< ent->TranslationParameter(j,k).Y().getValue() << ", Z="
		<< ent->TranslationParameter(j,k).Z().getValue() << endl;
	      S << "Rotational Parameter : X="
		<< ent->RotationalParameter(j,k).X().getValue() << ", Y="
		<< ent->RotationalParameter(j,k).Y().getValue() << ", Z="
		<< ent->RotationalParameter(j,k).Z().getValue();
	      if (k == nbcases) S << "] ";
	      S << endl;
	    }
	}
