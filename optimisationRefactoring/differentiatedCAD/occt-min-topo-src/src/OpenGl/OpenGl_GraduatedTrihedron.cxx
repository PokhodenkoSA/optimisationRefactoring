                                                      ...
                                                      const Standard_ShortReal theDpix) const
{
  ...
  if (aCurAspect.ToDrawName())
  {
    ...
    Standard_Real tmp;
    OpenGl_Vec3 aMiddle (theGridAxes.Ticks[theIndex] + aSizeVec * theGridAxes.Axes[theIndex] * 0.5f + aDir * (Standard_ShortReal)(tmp = (theDpix * anOffset)));
  ...
  if (aCurAspect.ToDrawValues() && aCurAspect.TickmarksNumber() > 0)
  {
    ...
    for (Standard_Integer anIt = 0; anIt <= aCurAspect.TickmarksNumber(); ++anIt)
    {
      ...
      OpenGl_Vec3 aPos (theGridAxes.Ticks[theIndex] + anAxis.Direction* (Standard_ShortReal) (anIt * aStep) + aDir * (Standard_ShortReal) (tmp = (theDpix * anOffset)));
