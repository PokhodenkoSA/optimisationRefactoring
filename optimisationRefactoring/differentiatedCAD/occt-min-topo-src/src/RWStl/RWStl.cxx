...
//      aMexp.TriangleOrientation (x,y,z);
      ...
      Sprintf (sval,
          ...
          "   endloop\n"
          " endfacet\n",
          Vnorm.X().getValue(), Vnorm.Y().getValue(), Vnorm.Z().getValue(),
          x1.getValue(), y1.getValue(), z1.getValue(),
          x2.getValue(), y2.getValue(), z2.getValue(),
          x3.getValue(), y3.getValue(), z3.getValue());
      buf += sval;
      theFile.Write (buf, buf.Length()); buf.Clear();
                                      ...
                                      const Handle(Message_ProgressIndicator)& theProgInd)
{
  ...
  // Filter unique vertices to share the nodes of the mesh.
  BRepBuilderAPI_CellFilter uniqueVertices((Standard_Real)Precision::Confusion());
  BRepBuilderAPI_VertexInspector inspector(Precision::Confusion());
