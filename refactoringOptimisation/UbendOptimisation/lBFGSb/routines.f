...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von if (task .eq. 'START') then nach if (task(1:5) .eq. 'START') then
c     ************
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'FG_START'  nach task(1:8) = 'FG_START' 
c     ************
      task(1:8) = 'FG_START' 
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'CONVERGENCE: NORM OF PROJECTED GRADIENT <= PGTOL'  nach task(1:48) = 'CONVERGENCE: NORM OF PROJECTED GRADIENT <= PGTOL'
c     ************
         task(1:48) = 'CONVERGENCE: NORM OF PROJECTED GRADIENT <= PGTOL'
         goto 999
      endif 
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'ABNORMAL_TERMINATION_IN_LNSRCH'  nach task(1:30) = 'ABNORMAL_TERMINATION_IN_LNSRCH'
c     ************
            task(1:30) = 'ABNORMAL_TERMINATION_IN_LNSRCH'
            iter = iter + 1
            goto 999
         else
c             refresh the lbfgs memory and restart the iteration.
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task   = 'RESTART_FROM_LNSRCH'  nach task(1:19)   = 'RESTART_FROM_LNSRCH'
c     ************
            task(1:19)   = 'RESTART_FROM_LNSRCH'
            call timer(cpu2)
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH'  nach task = 'CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH'
c     ************
         task(1:47) = 'CONVERGENCE: REL_REDUCTION_OF_F <= FACTR*EPSMCH'
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'ERROR: N .LE. 0' in task(:) = 'ERROR: N .LE. 0' fuer 3 ausdruecke
c     ************
      if (n .le. 0) task(1:15) = 'ERROR: N .LE. 0'
      if (m .le. 0) task(1:15) = 'ERROR: M .LE. 0'
      if (factr .lt. zero) task(1:19) = 'ERROR: FACTR .LT. 0'
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'ERROR: INVALID NBD' in task(1:18) = 'ERROR: INVALID NBD'
c     ************
            task(1:18) = 'ERROR: INVALID NBD'
	    info = -6
	    k = i
         endif
	 if (nbd(i) .eq. 2) then
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'ERROR: NO FEASIBLE SOLUTION' in task(1:27) = 'ERROR: NO FEASIBLE SOLUTION'
c     ************
               task(1:27) = 'ERROR: NO FEASIBLE SOLUTION'
	       info = -7
	       k = i
            endif
         endif
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von csave = 'START' in csave(1:5) = 'START'
c     ************
      csave(1:5) = 'START'
 556  continue
      gd = ddot(n,g,1,d,1)
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'FG_LNSRCH' in task(1:9) = 'FG_LNSRCH'
c     ************
	 task(1:9) = 'FG_LNSRCH'
	 ifun = ifun + 1
         nfgv = nfgv + 1
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'NEW_X' in task(1:5) = 'NEW_X'
c     ************
         task(1:5) = 'NEW_X'
      endif
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = .. nach task(:) = .. STPMIN
c     ************
         if (stp .lt. stpmin) task(1:22) = 'ERROR: STP .LT. STPMIN'
         if (stp .gt. stpmax) task(1:22) = 'ERROR: STP .GT. STPMAX'
         if (g .ge. zero) task(1:26) = 'ERROR: INITIAL G .GE. ZERO'
         if (ftol .lt. zero) task(1:21) = 'ERROR: FTOL .LT. ZERO'
         if (gtol .lt. zero) task(1:21) = 'ERROR: GTOL .LT. ZERO'
         if (xtol .lt. zero) task(1:21) = 'ERROR: XTOL .LT. ZERO'
         if (stpmin .lt. zero) task(1:23) = 'ERROR: STPMIN .LT. ZERO'
         if (stpmax .lt. stpmin) task(1:25) = 'ERROR: STPMAX .LT.'
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = 'FG' nach task(1:2) = 'FG'
c     ************
         task(1:2) = 'FG'
...
c     ************    21.04.2009 Hoffeins
c     C/Fortran-Problem mit Stringuebergabe (Laengenproblem)
c     Aenderung von task = .. nach task(:) = ..
c     ************
      if (brackt .and. (stp .le. stmin .or. stp .ge. stmax))
     +   task(1:41) = 'WARNING: ROUNDING ERRORS PREVENT PROGRESS'
      if (brackt .and. stmax - stmin .le. xtol*stmax) 
     +   task(1:28) = 'WARNING: XTOL TEST SATISFIED'
      if (stp .eq. stpmax .and. f .le. ftest .and. g .le. gtest) 
     +   task(1:21) = 'WARNING: STP = STPMAX'
      if (stp .eq. stpmin .and. (f .gt. ftest .or. g .ge. gtest)) 
     +   task(1:21) = 'WARNING: STP = STPMIN'

c     Test for convergence.

      if (f .le. ftest .and. abs(g) .le. gtol*(-ginit)) 
     +   task(1:11) = 'CONVERGENCE'
