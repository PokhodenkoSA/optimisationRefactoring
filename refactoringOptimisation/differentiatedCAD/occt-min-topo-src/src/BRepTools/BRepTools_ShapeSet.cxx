                                       ...
                                       Standard_OStream&   OS)const 
{
  ...
  if (S.ShapeType() == TopAbs_VERTEX) {
    ...
    OS << "    Tolerance : " << BRep_Tool::Tolerance(V).getValue() << "\n";
    gp_Pnt p = BRep_Tool::Pnt(V);
    OS << "    - Point 3D : "<<p.X().getValue()<<", "<<p.Y().getValue()<<", "<<p.Z().getValue()<<"\n";
    ...
    while (itrp.More()) {
      ...
      OS << "    - Parameter : "<< PR->Parameter().getValue();
      if (PR->IsPointOnCurve()) {
        ...
        OS << ", " << PR->Parameter2().getValue() << " on surface ";
    ...
    OS << "    Tolerance : " << TE->Tolerance().getValue() << "\n";
    ...
    while (itrc.More()) {
      ...
      if (CR->IsCurve3D()) {
        ...
        if (!CR->Curve3D().IsNull()) {
          ...
          OS <<", range : " << first.getValue() << " " << last.getValue() <<"\n";
        }
      }
      else if (CR->IsCurveOnSurface()) {
        Handle(BRep_GCurve) GC = Handle(BRep_GCurve)::DownCast(itrc.Value());
        ...
        COS->UVPoints(Pf,Pl);
        OS << "  UV Points : " <<Pf.X().getValue()<<", "<<Pf.Y().getValue()<<" ";
        OS << Pl.X().getValue()<<", "<<Pl.Y().getValue()<<"\n";
        if (CR->IsCurveOnClosedSurface()) {
    ...
    if (!TF->Surface().IsNull()) {
      OS << "    Tolerance : " <<TF->Tolerance().getValue() << "\n";
                                        ...
                                        Standard_OStream&   OS)const 
{
  ...
  if (S.ShapeType() == TopAbs_VERTEX) {
    ...
    TopoDS_Vertex V = TopoDS::Vertex(S);
    OS << BRep_Tool::Tolerance(V).getValue() << "\n";
    gp_Pnt p = BRep_Tool::Pnt(V);
    ...
    while (itrc.More()) {
      ...
      if (CR->IsCurve3D()) {
        if (!CR->Curve3D().IsNull()) {
          ...
          OS << " "<<first.getValue()<<" "<<last.getValue();
          OS << "\n";
        }
      }
      else if (CR->IsCurveOnSurface()) {
        ...
        if (FormatNb() == 2)
        {
          ...
          OS << Pf.X().getValue() << " " << Pf.Y().getValue() << " " << Pl.X().getValue() << " " << Pl.Y().getValue() << "\n";
        }
      }
      else if (CR->IsRegularity()) {
        OS << "4 ";                              // -4- Regularity
                                                     ...
                                                     const Standard_Boolean Compact)const
{
  ...
  for (i=1; i<=nbpOntri && PS.More(); i++, PS.Next()) {
    ...
    if (!Param.IsNull()) {
      ...
      if (!Compact) OS <<"  ";
      for (j=1; j <= Param->Length(); j++) OS << Param->Value(j).getValue() << " ";
      OS << "\n";
    }
    else OS <<"0 \n";
  }
                                        ...
                                        const Standard_Boolean Compact)const
{
  ...
  for (i = 1; i <= nbpol && PS.More(); i++, PS.Next()) {
    ...
    for (j = 1; j <= nbNodes; j++) {
      ...
      OS << Nodes(j).X().getValue() << " ";
      if (!Compact) OS << setw(17);
      OS << Nodes(j).Y().getValue() << " ";
      if (!Compact) OS << setw(17);
      OS << Nodes(j).Z().getValue();
      if (!Compact) OS << "\n";
      else OS << " ";
    }
    OS <<"\n";
  ...
  for (i = 1; i <= nbtri && PS.More(); i++, PS.Next()) {
    ...
    if (T->HasUVNodes()) {
      ...
      for (j = 1; j <= nbNodes; j++) {
        if (!Compact) OS << setw(10) << j << " : ";
        if (!Compact) OS << setw(17);
        OS << UVNodes(j).X().getValue() << " ";
        if (!Compact) OS << setw(17);
        OS << UVNodes(j).Y().getValue();
        if (!Compact) OS << "\n";
        else OS << " ";
      }
    }
