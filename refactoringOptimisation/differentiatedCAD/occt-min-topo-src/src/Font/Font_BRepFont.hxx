...
public:
  ...
  {
    return myScaleUnits * double(Font_FTFont::Ascender());
  }

  //! @return vertical distance from the horizontal baseline to the lowest character coordinate.
  double Descender() const
  {
    return myScaleUnits * double(Font_FTFont::Descender());
  }

  //! @return default line spacing (the baseline-to-baseline distance).
  double LineSpacing() const
  {
    return myScaleUnits * double(Font_FTFont::LineSpacing());
  }

  //! Configured point size
  double PointSize() const
  {
    return myScaleUnits * double(Font_FTFont::PointSize());
  }

  //! Compute advance to the next character with kerning applied when applicable.
  //! Assuming text rendered horizontally.
  double AdvanceX (const Standard_Utf32Char theUCharNext)
  {
    return myScaleUnits * double(Font_FTFont::AdvanceX (theUCharNext));
  }

  //! Compute advance to the next character with kerning applied when applicable.
  //! Assuming text rendered horizontally.
  double AdvanceX (const Standard_Utf32Char theUChar,
                          const Standard_Utf32Char theUCharNext)
  {
    return myScaleUnits * double(Font_FTFont::AdvanceX (theUChar, theUCharNext));
  }

  //! Compute advance to the next character with kerning applied when applicable.
  //! Assuming text rendered vertically.
  double AdvanceY (const Standard_Utf32Char theUCharNext)
  {
    return myScaleUnits * double(Font_FTFont::AdvanceY (theUCharNext));
  }

  //! Compute advance to the next character with kerning applied when applicable.
  //! Assuming text rendered vertically.
  double AdvanceY (const Standard_Utf32Char theUChar,
                          const Standard_Utf32Char theUCharNext)
  {
    return myScaleUnits * double(Font_FTFont::AdvanceY (theUChar, theUCharNext));
  }
...
private:
  ...
  {
    return gp_XY (myScaleUnits * double(theVec.x) / 64.0, myScaleUnits * double(theVec.y) / 64.0);
  }
