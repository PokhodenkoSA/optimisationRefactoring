             ...
             : theAlloc)
{
  if (theTriangulation.IsNull() == Standard_False) {
    ...
    if (theTriangulation->HasNormals()) {
      const TColStd_Array1OfReal& arrNorm = theTriangulation->Normals();
      for (i = 0; i < nNodes; i++) {
...
Handle(Poly_Triangulation) Poly_CoherentTriangulation::GetTriangulation() const
{
  ...
  if (nNodes > 0 && nTriangles > 0) {
    ...
    const Handle(TColStd_HArray1OfReal) harrNormal =
      new TColStd_HArray1OfReal(1, 3 * nNodes);
    Standard_Real * arrNormal = &harrNormal->ChangeValue(1);
    ...
    for (i = 0; i < myNodes.Length(); i++) {
      ...
      if (aNode.IsFreeNode())
        ...
        arrNormal[3 * aCount + 0] = static_cast<Standard_Real>(aNormal.X());
        arrNormal[3 * aCount + 1] = static_cast<Standard_Real>(aNormal.Y());
        arrNormal[3 * aCount + 2] = static_cast<Standard_Real>(aNormal.Z());
