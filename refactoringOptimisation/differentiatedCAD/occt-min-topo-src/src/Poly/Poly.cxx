                 ...
                 const Standard_Boolean Compact)
{
  ...
  for (i = 1; i <= nbNodes; i++) {
    ...
    OS << Nodes(i).X().getValue() << " ";
    if (!Compact) OS << setw(17);
    OS << Nodes(i).Y().getValue() << " ";
    if (!Compact) OS << setw(17);
    OS << Nodes(i).Z().getValue() << "\n";
  }
  ...
  if (T->HasUVNodes()) {
    ...
    for (i = 1; i <= nbNodes; i++) {
      if (!Compact) OS << setw(10) << i << " : ";
    if (!Compact) OS << setw(17);
      OS << UVNodes(i).X().getValue() << " ";
    if (!Compact) OS << setw(17);
      OS << UVNodes(i).Y().getValue() << "\n";
    }
  }
...
void  Poly::ComputeNormals(const Handle(Poly_Triangulation)& Tri)
{
  ...
  const Handle(TColStd_HArray1OfReal) Normals =
    new TColStd_HArray1OfReal(1, nbNormVal);
  Normals->Init(0.F);

  Standard_Real            * arrNormal = &(Normals->ChangeValue(1));
  ...
  for (iTri = 1; iTri <= arrTri.Length(); iTri++) {
    ...
    if (aMod > eps2) {
      ...
      iNode[1] = (iNode[1]-1)*3;
      iNode[2] = (iNode[2]-1)*3;
      arrNormal[iNode[0]+0] += (Standard_Real)aCoord[0];
      arrNormal[iNode[0]+1] += (Standard_Real)aCoord[1];
      arrNormal[iNode[0]+2] += (Standard_Real)aCoord[2];
      arrNormal[iNode[1]+0] += (Standard_Real)aCoord[0];
      arrNormal[iNode[1]+1] += (Standard_Real)aCoord[1];
      arrNormal[iNode[1]+2] += (Standard_Real)aCoord[2];
      arrNormal[iNode[2]+0] += (Standard_Real)aCoord[0];
      arrNormal[iNode[2]+1] += (Standard_Real)aCoord[1];
      arrNormal[iNode[2]+2] += (Standard_Real)aCoord[2];
    }
  }
  ...
  for (iN = 0; iN < nbNormVal; iN+=3) {
    ...
    if (aMod < eps2) {
      ...
      aMod = sqrt(aMod);
      arrNormal[iN+0] = Standard_Real(arrNormal[iN+0]/aMod);
      arrNormal[iN+1] = Standard_Real(arrNormal[iN+1]/aMod);
      arrNormal[iN+2] = Standard_Real(arrNormal[iN+2]/aMod);
    }
  }
