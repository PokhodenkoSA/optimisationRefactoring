...
void Message_ProgressIndicator::SetScale (const double min, 
                                           const double max, 
                                           const double step,
...
void Message_ProgressIndicator::GetScale (double &min, 
                                           double &max, 
                                           double &step,
...
void Message_ProgressIndicator::SetValue (const double val)
{
  const Message_ProgressScale &scale = myScopes(1);
  double p = scale.LocalToBase ( val );
  if ( myPosition < p ) {
    myPosition = Min ( p, 1. );
    Show(Standard_False);
  }
...
double Message_ProgressIndicator::GetValue () const
{
...
Standard_Boolean Message_ProgressIndicator::NewScope (const double span,
...
Standard_Boolean Message_ProgressIndicator::NextScope (const double span, 
...
//   double next = ( myMax - myMin <= gp::Resolution() ? 1. - myPosition :
