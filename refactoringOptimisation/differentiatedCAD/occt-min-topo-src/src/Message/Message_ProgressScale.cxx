...
static const double INFINITE = 1e100;
...
double Message_ProgressScale::LocalToBase (const double val) const
{
  ...
  double x = ( val - myMin ) / ( myMax - myMin );
...
double Message_ProgressScale::BaseToLocal (const double val) const
{
  if ( myLast - val <= ZERO ) 
    return myInfinite ? INFINITE : myMax;
  if ( ! myInfinite )
    return myMin + ( myMax - myMin ) * ( val - myFirst ) / ( myLast - myFirst );
//  double x = log ( ( val - myFirst ) / ( myLast - val ) ); // exponent
  double x = ( val - myFirst ) / ( myLast - val );         // hyperbola
