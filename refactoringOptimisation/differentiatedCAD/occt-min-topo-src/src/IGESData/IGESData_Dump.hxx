...
#define IGESData_DumpXY(S,XYval) \
 S << " (" << XYval.X().getValue() << "," << XYval.Y().getValue() << ")"

#define IGESData_DumpXYZ(S,XYZval) \
  S << " (" << XYZval.X().getValue() << "," << XYZval.Y().getValue() << "," << XYZval.Z().getValue() << ")"
...
#define IGESData_DumpValsAD(S,Level,lower,upper,item) \
{\
  Standard_Integer lo = lower;  Standard_Integer up = upper;\
  IGESData_DumpListHeader(S,lo,up);\
  if (lo > up) {}\
    ...
    S << " :";\
    for (Standard_Integer iopa = lo; iopa <= up; iopa ++)  S << " " << item(iopa).getValue();\
  }\
}
...
{\
  ...
  if (loco > upc || lor > upr) {}\
    ...
    for (int ir = lor; ir <= upr; ir ++) {\
      S << "Row "<<ir<<":[";\
      for (int ic = loco; ic <= upc; ic ++)  S << " " << Item(ic,ir).getValue();\
      S << " ]\n";\
    }\
  }\
}
