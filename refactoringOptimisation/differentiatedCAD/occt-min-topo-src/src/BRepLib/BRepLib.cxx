  ...
  const Adaptor3d_CurveOnSurface& aCurveOnSurface)
{
  ...
  Standard_Integer aReturn = (Standard_Integer) (  30 + Max(aNbSKnots, aNbC2dKnots) ).getValue() ;
  return aReturn;
}
                                    ...
                                    const Standard_Boolean theForceComputeNormals)
{
  ...
  for (; anExpFace.More(); anExpFace.Next())
  {
    ...
    Handle(TColStd_HArray1OfReal) aNormArr = new TColStd_HArray1OfReal(1, anArrDim);
    ...
    for(Standard_Integer i = aPT->UVNodes().Lower(); i <= aPT->UVNodes().Upper(); i++)
    {
      ...
      aNormArr->ChangeValue(anNormInd++) = aNorm.X();
      aNormArr->ChangeValue(anNormInd++) = aNorm.Y();
      aNormArr->ChangeValue(anNormInd++) = aNorm.Z();
    }
  ...
  for(Standard_Integer anInd = 1; anInd <= aMapEF.Extent(); anInd++)
  {
    ...
    TColStd_Array1OfReal& aNormArr1 = aPT1->ChangeNormals();
    TColStd_Array1OfReal& aNormArr2 = aPT2->ChangeNormals();
