...
//	     const GeomAbs_SurfaceType TypeS2,
  ...
  switch (typl) {
    ...
    case IntPatch_Walking: {  //-- ****************************************
      ...
      Standard_Integer ParamMinOnLine = (Standard_Integer) WLine->Vertex(i).ParameterOnLine().getValue();
      Standard_Integer ParamMaxOnLine = (Standard_Integer) WLine->Vertex(j).ParameterOnLine().getValue();
				       ...
				       const Standard_Real TolArc*/)
{
  ...
  Standard_Integer ipF = (Standard_Integer) vtxF.ParameterOnLine().getValue();
  Standard_Integer ipL = (Standard_Integer) vtxL.ParameterOnLine().getValue();
...
static void TestWLineToRLine(const IntPatch_SequenceOfLine& slinref,
  ...
  if (nbvtx < 2) return;
  Standard_Integer ParamMinOnLine = (Standard_Integer) WLine->Vertex(1).ParameterOnLine().getValue();
  Standard_Integer ParamMaxOnLine = (Standard_Integer) WLine->Vertex(nbvtx).ParameterOnLine().getValue();
  ...
  for (iv=1; iv <= nbvtx; iv++) {
    Standard_Integer plin = (Standard_Integer) WLine->Vertex(iv).ParameterOnLine().getValue();
  ...
  for (is=0; is<2; is++) {
	...
	if (!(WLine->Vertex(iv).*pIsOnDomS)()) {
	  Standard_Integer ip = (Standard_Integer) WLine->Vertex(iv).ParameterOnLine().getValue();
...
void IntPatch_LineConstructor::Perform(const IntPatch_SequenceOfLine& slinref,
  ...
  if(typl == IntPatch_Analytic) { 
	...
	pmid = (Standard_Integer)((firstp+lastp)/2).getValue();
	Standard_Integer int_lastp = (Standard_Integer)lastp.getValue();
	Standard_Integer int_firstp = (Standard_Integer)firstp.getValue();
	...
	if (in1 != TopAbs_OUT && in2 != TopAbs_OUT)
	  {
	    ...
	    const IntSurf_PntOn2S& Pmid1 = WLine->Point((Standard_Integer)firstp.getValue());
	    Pmid1.Parameters(u1a,v1a,u2a,v2a);
	    Recadre(mySurf1,mySurf2,u1a,v1a,u2a,v2a);
	    
	    const IntSurf_PntOn2S& Pmid2 = WLine->Point((Standard_Integer)lastp.getValue());
