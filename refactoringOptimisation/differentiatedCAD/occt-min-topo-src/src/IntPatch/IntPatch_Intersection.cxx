                                ...
                                const Standard_Real thePeriod)
{
  ...
  const Standard_Integer aPar1 = static_cast<Standard_Integer>(Floor(aVal1).getValue());
  const Standard_Integer aPar2 = static_cast<Standard_Integer>(Floor(aVal2).getValue());
  if(aPar1 != aPar2)
                        ...
                        Handle(Geom_Curve)&         I)
{
  ...
  else//OffsetSurface
  {
    ...
    else//Offset Line, Parab, Hyperb
    {
      ...
      {
        Standard_Real tmp;
        if(FirstV >= 0. && LastV >= 0.){ VmTr = FirstV; VMTr = ((LastV - FirstV) > 1.e+4) ? (tmp = (FirstV + 1.e+4)) : LastV; }
        else if(FirstV < 0. && LastV < 0.){ VMTr = LastV; VmTr = ((FirstV - LastV) < -1.e+4) ? (tmp = (LastV - 1.e+4)) : FirstV; }
        else { VmTr = (FirstV < -1.e+4) ? -1.e+4 : FirstV; VMTr = (LastV > 1.e+4) ? (tmp = 1.e+4) : LastV; }
      }
      else//Hyperbola
      {
        if(FirstV >= 0. && LastV >= 0.)
                        ...
                        Handle(Geom_Curve)&         I)
{
  ...
  else//OffsetSurface
  {
    ...
    else//Offset Line, Parab, Hyperb
    {
      ...
      {
        Standard_Real tmp;
        if(FirstU >= 0. && LastU >= 0.){ UmTr = FirstU; UMTr = ((LastU - FirstU) > 1.e+4) ? (tmp = (FirstU + 1.e+4)) : LastU; }
        else if(FirstU < 0. && LastU < 0.){ UMTr = LastU; UmTr = ((FirstU - LastU) < -1.e+4) ? (tmp = (LastU - 1.e+4)) : FirstU; }
        else { UmTr = (FirstU < -1.e+4) ? -1.e+4 : FirstU; UMTr = (LastU > 1.e+4) ? (tmp = 1.e+4) : LastU; }
      }
      else//Hyperbola
      {
        if(FirstU >= 0. && LastU >= 0.)
    ...
    case GeomAbs_Parabola:
    {
      if(Abs(Lst - Fst) > TrVal)
      {
        if(Fst >= 0. && Lst >= 0.)
        {
          Standard_Real tmp;
          NewFst = Fst;
          NewLst = ((Fst + TrVal) < Lst) ? (tmp = (Fst + TrVal)) : Lst;
        }
        if(Fst < 0. && Lst < 0.)
        {
          Standard_Real tmp;
          NewLst = Lst;
          NewFst = ((Lst - TrVal) > Fst) ? (tmp = (Lst - TrVal)) : Fst;
        }
        else
        {
          Standard_Real tmp;
          NewFst = (Fst < -TrVal) ? (tmp = -TrVal) : Fst;
