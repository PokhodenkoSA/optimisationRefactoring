   ...
   const Standard_Boolean ShowKnots
    ...
    if (Degree == 1) {    
      ...
      if (I1 > ka)  { ka = I1;  Uk1 = U1; }
        ...
        U = U1;
        NbPoints = (Standard_Integer) Abs (Discret * (U1 - Ustart) / (Ustart - Uend)).getValue();
      ...
      if (J2 < kb)  { kb = J2;  Uk2 = U2; }
        ...
        Uk2 = Uend;
        U   = Uend;
        NbPoints = (Standard_Integer) Abs (Discret * (U2 - Uend) / (Ustart - Uend)).getValue();
        NbPoints = Max (NbPoints, 30);
        Du = (U2 - Uend) / NbPoints;
        dis.MoveTo(C->Value (U));
        for (Standard_Integer i = 1; i <= NbPoints - 2; i++) {
      ...
      for (Standard_Integer k = ka; k < kb; k++) {
        ...
        U  = Ua;
        NbPoints = (Standard_Integer) Abs (Discret * (Ua - Ub) / (Ustart - Uend)).getValue();
        NbPoints = Max (NbPoints, 30);
        Du = (Ub - Ua) / NbPoints;
        dis.MoveTo(C->Value (U));
        for (Standard_Integer i = 1; i <= NbPoints - 2; i++) {
