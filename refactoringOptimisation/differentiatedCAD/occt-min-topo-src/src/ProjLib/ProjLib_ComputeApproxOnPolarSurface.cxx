                               ...
                               const aFuncStruct& theData)
{
    ...
    case GeomAbs_Cylinder:
      {
        ...
        if(U0 < Uinf) decalU = -int((Uinf - U0).getValue()/(2*M_PI))-1;
        if(U0 > Usup) decalU =  int((U0 - Usup).getValue()/(2*M_PI))+1;
        S += decalU*2*M_PI;
        break;
      }
    case GeomAbs_Cone:
    ...
    case GeomAbs_Sphere:
      {
        ...
        if(U0 > Usup) decalU =  int((U0 - Usup).getValue()/(2*M_PI))+1;
        S += decalU*2*M_PI;
        if(V0 < Vinf) decalV = -int((Vinf - V0).getValue()/(2*M_PI))-1;
        if(V0 > (Vsup+(Vsup-Vinf))) decalV =  int((V0 - Vsup+(Vsup-Vinf)).getValue()/(2*M_PI))+1;
        T += decalV*2*M_PI;
    ...
    case GeomAbs_Torus:
      {
        ...
        if(U0 > Usup) decalU =  int((U0 - Usup).getValue()/(2*M_PI))+1;
        if(V0 < Vinf) decalV = -int((Vinf - V0).getValue()/(2*M_PI))-1;
        if(V0 > Vsup) decalV =  int((V0 - Vsup).getValue()/(2*M_PI))+1;
  ...
  if(U0 < Uinf)
  {
    ...
    {
      decalU = int(((Uinf - U0)/uperiod).getValue())+1;
      U0 += decalU*uperiod;
    }
  }
  if(U0 > Usup)
  {
    ...
    {
      decalU = -(int(((U0 - Usup)/uperiod).getValue())+1);
      U0 += decalU*uperiod;
    }
  }
  if(V0 < Vinf)
  {
    ...
    {
      decalV = int(((Vinf - V0)/vperiod).getValue())+1;
      V0 += decalV*vperiod;
    }
  }
  if(V0 > Vsup)
  {
    ...
    {
      decalV = -int(((V0 - Vsup)/vperiod).getValue())-1;
      V0 += decalV*vperiod;
    }
  }
...
//	gp_Pnt pntproj;
	if(TheTypeC == GeomAbs_BSplineCurve) {
	  ...
	  for(i = 1;i <= Curve->NbPoles();i++) {
	    ...
	    if (extrloc.IsDone()) {
	      Dist2Min = (Standard_Integer ) extrloc.SquareDistance().getValue();
