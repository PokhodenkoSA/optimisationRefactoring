...
void AppParCurves_MultiPoint::Dump(Standard_OStream& o) const
{
  ...
  if(aNbPnts3D > 0)
  {
    for(Standard_Integer i = tabPoint->Lower(); i <= tabPoint->Upper(); i++)
    {
      ...
      o << " Pole x = " << (tabPoint->Value(i)/*->Point(j)*/).X().getValue() << endl;
      o << " Pole y = " << (tabPoint->Value(i)/*->Point(j)*/).Y().getValue() << endl;
      o << " Pole z = " << (tabPoint->Value(i)/*->Point(j)*/).Z().getValue() << endl;
    }
  }
  ...
  if(aNbPnts2D > 0)
  {
    for(Standard_Integer i = tabPoint2d->Lower(); i <= tabPoint2d->Upper(); i++)
    {
      ...
      o << " Pole x = " << (tabPoint2d->Value(i)/*->Point2d(j)*/).X().getValue() << endl;
      o << " Pole y = " << (tabPoint2d->Value(i)/*->Point2d(j)*/).Y().getValue() << endl;
    }
  }
}
