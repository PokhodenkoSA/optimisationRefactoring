...
Standard_Integer BOUNDS(Draw_Interpretor& di, Standard_Integer narg, const char** a)
{
  ...
  Draw::Set(a[2],xmin.getValue());
  Draw::Set(a[3],ymin.getValue());
  Draw::Set(a[4],zmin.getValue());
  Draw::Set(a[5],xmax.getValue());
  Draw::Set(a[6],ymax.getValue());
  Draw::Set(a[7],zmax.getValue());
  di<<"xmin:"<<xmin.getValue()<<" ";
  di<<"ymin:"<<ymin.getValue()<<" ";
  di<<"zmin:"<<zmin.getValue()<<" ";
  di<<"xmax:"<<xmax.getValue()<<" ";
  di<<"ymax:"<<ymax.getValue()<<" ";
  di<<"zmax:"<<zmax.getValue()<<"\n";
  return 0 ;
}
...
Standard_Integer DOTVIEW(Draw_Interpretor&, Standard_Integer narg, const char** a)
{
  ...
  gp_Dir d1(dx,dy,dz);
  double idr; Draw::Get(a[4],idr);
  gp_Pnt p; gp_Dir d2; PersEyeDir((Standard_Integer)idr,p,d2);
  
  gp_Dir d3(d1.Crossed(d2));
  Draw::Set(a[5],d3.X().getValue());
  Draw::Set(a[6],d3.Y().getValue());
  Draw::Set(a[7],d3.Z().getValue());
...
Standard_Integer VECTEUR(Draw_Interpretor& di, Standard_Integer , const char** ) {
  ...
  dout.Flush();
  di<<"\n";
  di<<"cdinp P1 "<<P1.X().getValue()<<" "<<P1.Y().getValue()<<" "<<P1.Z().getValue()<<"\n";
  di<<"cdinp P2 "<<PP1.X().getValue()<<" "<<PP1.Y().getValue()<<" "<<PP1.Z().getValue()<<"\n";
  di<<"cdinp PM "<<((PP1.X()+P1.X())/2).getValue()<<" "<<((PP1.Y()+P1.Y())/2).getValue()<<" "<<((PP1.Z()+P1.Z())/2).getValue()<<"\n";
  di<<"\n";
  di<<"ttranslate "<<(PP1.X()-P1.X()).getValue()<<" "<<(PP1.Y()-P1.Y()).getValue()<<" "<<(PP1.Z()-P1.Z()).getValue()<<"\n";
...
Standard_Integer GRILLE(Draw_Interpretor& di, Standard_Integer narg, const char** a)
{
  ...
  for(Standard_Real x=xmin; x<xmax; x+=dx) { 
    for(Standard_Real y=ymin; y<ymax; y+=dx) { 
      ...
      di<<"line l "<<x.getValue()<<" "<<y.getValue()<<" "<<zmin.getValue()<<" 0 0 1;mkedge e l 0 ";
      di<<(zmax-zmin).getValue()<<" ;orient e EXTERNAL; add e CE"<<"\n";
      if(dy==0) y=ymax;
    } 
    if(dx==0) x=xmax;
  }
...
Standard_Integer GETP3D(Draw_Interpretor& di, Standard_Integer /*narg*/, const char** a)
{
  ...
  di<<pV.X().getValue()<<" "<<pV.Y().getValue()<<" "<<pV.Z().getValue()<<"\n";
  return 0;
}
#ifdef OCCT_DEBUG
