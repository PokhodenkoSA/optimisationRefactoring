...
namespace
{
  ...
  static const double MIN_DISTANCE = Pow (0.1, ShortRealDigits() - 2);
...
static Graphic3d_Vec4d safePointCast (const gp_Pnt& thePnt)
{
  ...
  if (Abs (aSafePoint.X()) > aLim)
    aSafePoint.SetX (aSafePoint.X() >= 0 ? aBigFloat : (tmp = -aBigFloat));
  if (Abs (aSafePoint.Y()) > aLim)
    aSafePoint.SetY (aSafePoint.Y() >= 0 ? aBigFloat : (tmp = -aBigFloat));
  if (Abs (aSafePoint.Z()) > aLim)
    aSafePoint.SetZ (aSafePoint.Z() >= 0 ? aBigFloat : (tmp = -aBigFloat));
...
gp_XYZ Graphic3d_Camera::ViewDimensions() const
{
  ...
  Standard_Real tmp;
  Standard_Real aSize = IsOrthographic() ? myScale : (tmp = (2.0 * Distance() * Tan (DTR_HALF * myFOVy)));
  ...
  Graphic3d_Camera::UpdateProjection (TransformMatrices<Elem_t>& theMatrices) const
{
  ...
  Elem_t aScale   = static_cast<Elem_t> (myScale.getValue());
  Elem_t aZNear   = static_cast<Elem_t> (myZNear.getValue());
  Elem_t aZFar    = static_cast<Elem_t> (myZFar.getValue());
  Elem_t anAspect = static_cast<Elem_t> (myAspect.getValue());
  ...
  {
    aDXHalf = aZNear * Elem_t (Tan (DTR_HALF * myFOVy).getValue());
    aDYHalf = aZNear * Elem_t (Tan (DTR_HALF * myFOVy).getValue());
  }
  ...
  Elem_t aIOD  = myIODType == IODType_Relative 
    ? static_cast<Elem_t> ((myIOD * Distance()).getValue())
    : static_cast<Elem_t> (myIOD.getValue());

  Elem_t aFocus = myZFocusType == FocusType_Relative 
    ? static_cast<Elem_t> ((myZFocus * Distance()).getValue())
    : static_cast<Elem_t> (myZFocus.getValue());
  ...
  Graphic3d_Camera::UpdateOrientation (TransformMatrices<Elem_t>& theMatrices) const
{
  ...
  NCollection_Vec3<Elem_t> anEye (static_cast<Elem_t> (myEye.X().getValue()),
                                  static_cast<Elem_t> (myEye.Y().getValue()),
                                  static_cast<Elem_t> (myEye.Z().getValue()));

  NCollection_Vec3<Elem_t> aCenter (static_cast<Elem_t> (myCenter.X().getValue()),
                                    static_cast<Elem_t> (myCenter.Y().getValue()),
                                    static_cast<Elem_t> (myCenter.Z().getValue()));

  NCollection_Vec3<Elem_t> anUp (static_cast<Elem_t> (myUp.X().getValue()),
                                 static_cast<Elem_t> (myUp.Y().getValue()),
                                 static_cast<Elem_t> (myUp.Z().getValue()));

  NCollection_Vec3<Elem_t> anAxialScale (static_cast<Elem_t> (myAxialScale.X().getValue()),
                                         static_cast<Elem_t> (myAxialScale.Y().getValue()),
                                         static_cast<Elem_t> (myAxialScale.Z().getValue()));
                                      ...
                                      NCollection_Mat4<Elem_t>& theOutMx)
{
  ...
  Elem_t tmp1 = theLeft  + aDXStereoShift;
  Elem_t tmp2 = theRight + aDXStereoShift;
  PerspectiveProj (tmp1,
                   tmp2,
