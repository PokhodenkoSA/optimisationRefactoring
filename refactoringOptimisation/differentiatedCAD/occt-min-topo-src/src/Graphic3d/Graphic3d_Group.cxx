...
{
  myBounds = Graphic3d_BndBox4f (Graphic3d_Vec4 (static_cast<Standard_ShortReal> (theXMin.getValue()),
                                                 static_cast<Standard_ShortReal> (theYMin.getValue()),
                                                 static_cast<Standard_ShortReal> (theZMin.getValue()),
                                                 1.0f),
                                 Graphic3d_Vec4 (static_cast<Standard_ShortReal> (theXMax.getValue()),
                                                 static_cast<Standard_ShortReal> (theYMax.getValue()),
                                                 static_cast<Standard_ShortReal> (theZMax.getValue()),
                                                 1.0f));
}
...
void Graphic3d_Group::SetGroupPrimitivesAspect (const Handle(Graphic3d_AspectFillArea3d)& theAspFill)
{
  ...
  // Material properties
  ContextFillArea.Back.Shininess       = float (aBack.Shininess().getValue());
  ContextFillArea.Back.Ambient         = float (aBack.Ambient().getValue());
  ContextFillArea.Back.Diffuse         = float (aBack.Diffuse().getValue());
  ContextFillArea.Back.Specular        = float (aBack.Specular().getValue());
  ContextFillArea.Back.Transparency    = float (aBack.Transparency().getValue());
  ContextFillArea.Back.Emission        = float (aBack.Emissive().getValue());
  ContextFillArea.Back.RefractionIndex = float (aBack.RefractionIndex().getValue());
  ...
  // Material properties
  ContextFillArea.Front.Shininess       = float (aFront.Shininess().getValue());
  ContextFillArea.Front.Ambient         = float (aFront.Ambient().getValue());
  ContextFillArea.Front.Diffuse         = float (aFront.Diffuse().getValue());
  ContextFillArea.Front.Specular        = float (aFront.Specular().getValue());
  ContextFillArea.Front.Transparency    = float (aFront.Transparency().getValue());
  ContextFillArea.Front.Emission        = float (aFront.Emissive().getValue());
  ContextFillArea.Front.RefractionIndex = float (aFront.RefractionIndex().getValue());
...
void Graphic3d_Group::SetGroupPrimitivesAspect (const Handle(Graphic3d_AspectText3d)& theAspText)
{
  ...
  ContextText.Expan           = float (anExpansion.getValue());
  ContextText.Space           = float (aSpace.getValue());
  ...
  ContextText.TextAngle       = float (aTextAngle.getValue());
                                             ...
                                             const Handle(Graphic3d_AspectFillArea3d)& theAspFill) const
{
  ...
  aColor.SetValues (double (anAspLine.Color.r),
                    double (anAspLine.Color.g),
                    double (anAspLine.Color.b), Quantity_TOC_RGB);
  theAspLine->SetColor         (aColor);
  theAspLine->SetType          (Aspect_TypeOfLine (anAspLine.LineType));
  theAspLine->SetWidth         (double     (anAspLine.Width));
  ...
  aColor.SetValues (double (anAspText.Color.r),
                    double (anAspText.Color.g),
                    double (anAspText.Color.b), Quantity_TOC_RGB);
  theAspText->SetColor (aColor);
  aColor.SetValues (double (anAspText.ColorSubTitle.r),
                    double (anAspText.ColorSubTitle.g),
                    double (anAspText.ColorSubTitle.b), Quantity_TOC_RGB);
  ...
  aColor.SetValues (double (anAspMarker.Color.r),
                    double (anAspMarker.Color.g),
                    double (anAspMarker.Color.b), Quantity_TOC_RGB);
  theAspMarker->SetColor (aColor);
  theAspMarker->SetType  (anAspMarker.MarkerType);
  theAspMarker->SetScale (double (anAspMarker.Scale));
  ...
  aColor.SetValues (double (anAspFill.IntColor.r),
                    double (anAspFill.IntColor.g),
                    double (anAspFill.IntColor.b), Quantity_TOC_RGB);
  theAspFill->SetInteriorColor (aColor);

  // Edges
  aColor.SetValues (double (anAspFill.EdgeColor.r),
                    double (anAspFill.EdgeColor.g),
                    double (anAspFill.EdgeColor.b), Quantity_TOC_RGB);
  theAspFill->SetEdgeColor    (aColor);
  theAspFill->SetEdgeLineType (Aspect_TypeOfLine (anAspFill.LineType));
  theAspFill->SetEdgeWidth    (double     (anAspFill.Width));
  ...
  aColor.SetValues (double (anAspFill.Back.ColorSpec.r),
                    double (anAspFill.Back.ColorSpec.g),
                    double (anAspFill.Back.ColorSpec.b), Quantity_TOC_RGB);
  aBack.SetSpecularColor (aColor);

  aColor.SetValues (double (anAspFill.Back.ColorAmb.r),
                    double (anAspFill.Back.ColorAmb.g),
                    double (anAspFill.Back.ColorAmb.b), Quantity_TOC_RGB);
  aBack.SetAmbientColor (aColor);

  aColor.SetValues (double (anAspFill.Back.ColorDif.r),
                    double (anAspFill.Back.ColorDif.g),
                    double (anAspFill.Back.ColorDif.b), Quantity_TOC_RGB);
  aBack.SetDiffuseColor (aColor);

  aColor.SetValues (double (anAspFill.Back.ColorEms.r),
                    double (anAspFill.Back.ColorEms.g),
                    double (anAspFill.Back.ColorEms.b), Quantity_TOC_RGB);
  ...
  aColor.SetValues (double (anAspFill.Front.ColorSpec.r),
                    double (anAspFill.Front.ColorSpec.g),
                    double (anAspFill.Front.ColorSpec.b), Quantity_TOC_RGB);
  aFront.SetSpecularColor (aColor);

  aColor.SetValues (double (anAspFill.Front.ColorAmb.r),
                    double (anAspFill.Front.ColorAmb.g),
                    double (anAspFill.Front.ColorAmb.b), Quantity_TOC_RGB);
  aFront.SetAmbientColor (aColor);

  aColor.SetValues (double (anAspFill.Front.ColorDif.r),
                    double (anAspFill.Front.ColorDif.g),
                    double (anAspFill.Front.ColorDif.b), Quantity_TOC_RGB);
  aFront.SetDiffuseColor (aColor);

  aColor.SetValues (double (anAspFill.Front.ColorEms.r),
                    double (anAspFill.Front.ColorEms.g),
                    double (anAspFill.Front.ColorEms.b), Quantity_TOC_RGB);
                            ...
                            const Standard_Boolean                  theToEvalMinMax)
{
  ...
  if (theToEvalMinMax)
  {
    ...
    myBounds.Add (Graphic3d_Vec4 (static_cast<Standard_ShortReal> (theOrientation.Location().X().getValue()),
                                  static_cast<Standard_ShortReal> (theOrientation.Location().Y().getValue()),
                                  static_cast<Standard_ShortReal> (theOrientation.Location().Z().getValue()),
                                  1.0f));
  }
  Update();
}
