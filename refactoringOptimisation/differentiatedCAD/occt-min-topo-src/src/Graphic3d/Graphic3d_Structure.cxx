...
{
  const double anRGB[3] =
  {
    double (myCStructure->ContextLine.Color.r),
    double (myCStructure->ContextLine.Color.g),
    double (myCStructure->ContextLine.Color.b)
  };
...
Handle(Graphic3d_AspectText3d) Graphic3d_Structure::Text3dAspect() const
{
  const double anRGB[3] =
  {
    double (myCStructure->ContextText.Color.r),
    double (myCStructure->ContextText.Color.g),
    double (myCStructure->ContextText.Color.b)
  };
  Quantity_Color aColor;
  aColor.SetValues (anRGB[0], anRGB[1], anRGB[2], Quantity_TOC_RGB);
...
Handle(Graphic3d_AspectMarker3d) Graphic3d_Structure::Marker3dAspect() const
{
  const double anRGB[3] =
  {
    double (myCStructure->ContextMarker.Color.r),
    double (myCStructure->ContextMarker.Color.g),
    double (myCStructure->ContextMarker.Color.b)
  };
  Quantity_Color aColor;
  aColor.SetValues (anRGB[0], anRGB[1], anRGB[2], Quantity_TOC_RGB);
...
Handle(Graphic3d_AspectFillArea3d) Graphic3d_Structure::FillArea3dAspect() const
{
  ...
  Quantity_Color aColor (double (myCStructure->ContextFillArea.Back.ColorSpec.r),
                         double (myCStructure->ContextFillArea.Back.ColorSpec.g),
                         double (myCStructure->ContextFillArea.Back.ColorSpec.b), Quantity_TOC_RGB);
  aBack.SetSpecularColor (aColor);

  aColor.SetValues (double (myCStructure->ContextFillArea.Back.ColorAmb.r),
                    double (myCStructure->ContextFillArea.Back.ColorAmb.g),
                    double (myCStructure->ContextFillArea.Back.ColorAmb.b), Quantity_TOC_RGB);
  aBack.SetAmbientColor (aColor);

  aColor.SetValues (double (myCStructure->ContextFillArea.Back.ColorDif.r),
                    double (myCStructure->ContextFillArea.Back.ColorDif.g),
                    double (myCStructure->ContextFillArea.Back.ColorDif.b), Quantity_TOC_RGB);
  aBack.SetDiffuseColor (aColor);

  aColor.SetValues (double (myCStructure->ContextFillArea.Back.ColorEms.r),
                    double (myCStructure->ContextFillArea.Back.ColorEms.g),
                    double (myCStructure->ContextFillArea.Back.ColorEms.b), Quantity_TOC_RGB);
  ...
  aColor.SetValues (double (myCStructure->ContextFillArea.Front.ColorSpec.r),
                    double (myCStructure->ContextFillArea.Front.ColorSpec.g),
                    double (myCStructure->ContextFillArea.Front.ColorSpec.b), Quantity_TOC_RGB);
  aFront.SetSpecularColor (aColor);

  aColor.SetValues (double (myCStructure->ContextFillArea.Front.ColorAmb.r),
                    double (myCStructure->ContextFillArea.Front.ColorAmb.g),
                    double (myCStructure->ContextFillArea.Front.ColorAmb.b), Quantity_TOC_RGB);
  aFront.SetAmbientColor (aColor);

  aColor.SetValues (double (myCStructure->ContextFillArea.Front.ColorDif.r),
                    double (myCStructure->ContextFillArea.Front.ColorDif.g),
                    double (myCStructure->ContextFillArea.Front.ColorDif.b), Quantity_TOC_RGB);
  aFront.SetDiffuseColor (aColor);

  aColor.SetValues (double (myCStructure->ContextFillArea.Front.ColorEms.r),
                    double (myCStructure->ContextFillArea.Front.ColorEms.g),
                    double (myCStructure->ContextFillArea.Front.ColorEms.b), Quantity_TOC_RGB);
  ...
  Quantity_Color anIntColor  (double (myCStructure->ContextFillArea.IntColor.r),
                              double (myCStructure->ContextFillArea.IntColor.g),
                              double (myCStructure->ContextFillArea.IntColor.b), Quantity_TOC_RGB);
  Quantity_Color anEdgeColor (double (myCStructure->ContextFillArea.EdgeColor.r),
                              double (myCStructure->ContextFillArea.EdgeColor.g),
                              double (myCStructure->ContextFillArea.EdgeColor.b), Quantity_TOC_RGB);
...
void Graphic3d_Structure::SetPrimitivesAspect (const Handle(Graphic3d_AspectFillArea3d)& theAspFill)
{
  ...
  // Light specificity
  myCStructure->ContextFillArea.Back.Shininess       = float (aBack.Shininess().getValue());
  myCStructure->ContextFillArea.Back.Ambient         = float (aBack.Ambient().getValue());
  myCStructure->ContextFillArea.Back.Diffuse         = float (aBack.Diffuse().getValue());
  myCStructure->ContextFillArea.Back.Specular        = float (aBack.Specular().getValue());
  myCStructure->ContextFillArea.Back.Transparency    = float (aBack.Transparency().getValue());
  myCStructure->ContextFillArea.Back.RefractionIndex = float (aBack.RefractionIndex().getValue());
  myCStructure->ContextFillArea.Back.BSDF            = aBack.BSDF();
  myCStructure->ContextFillArea.Back.Emission        = float (aBack.Emissive().getValue());
  ...
  // Light specificity
  myCStructure->ContextFillArea.Front.Shininess       = float (aFront.Shininess().getValue());
  myCStructure->ContextFillArea.Front.Ambient         = float (aFront.Ambient().getValue());
  myCStructure->ContextFillArea.Front.Diffuse         = float (aFront.Diffuse().getValue());
  myCStructure->ContextFillArea.Front.Specular        = float (aFront.Specular().getValue());
  myCStructure->ContextFillArea.Front.Transparency    = float (aFront.Transparency().getValue());
  myCStructure->ContextFillArea.Front.RefractionIndex = float (aFront.RefractionIndex().getValue());
  myCStructure->ContextFillArea.Front.BSDF            = aFront.BSDF();
  myCStructure->ContextFillArea.Front.Emission        = float (aFront.Emissive().getValue());
...
void Graphic3d_Structure::SetPrimitivesAspect (const Handle(Graphic3d_AspectText3d)& theAspText)
{
  ...
  myCStructure->ContextText.Expan           = float (anExpansion.getValue());
  myCStructure->ContextText.Space           = float (aSpace.getValue());
  ...
  myCStructure->ContextText.TextAngle       = float (aTextAngle.getValue());
  ...
  switch (theType)
  {
    case Graphic3d_TOC_REPLACE:
    {
      ...
      for (Standard_Integer i = 0; i <= 3; ++i)
      {
        ...
        {
          myCStructure->Transformation[i][j] = float (theMatrix (lr + i, lc + j).getValue());
    ...
    case Graphic3d_TOC_POSTCONCATENATE:
    {
      ...
      for (Standard_Integer i = 0; i <= 3; ++i)
      {
        for (Standard_Integer j = 0; j <= 3; ++j)
        {
          myCStructure->Transformation[i][j] = float (aNewTrsf (i, j).getValue());
        }
      }
      break;
    }
...
{
  Standard_CString          aFont;
  Standard_Real             aSpace, anExpansion;
  double aWidth, aScale;
...
void Graphic3d_Structure::GraphicTransform (const TColStd_Array2OfReal& theMatrix)
{
  for (Standard_Integer i = 0; i <= 3; ++i)
  {
    for (Standard_Integer j = 0; j <= 3; ++j)
    {
      myCStructure->Transformation[i][j] = float (theMatrix (i, j).getValue());
    }
  }
  myCStructure->UpdateTransformation();
}
