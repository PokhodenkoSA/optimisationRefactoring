...
//    Standard_Real par = Params->Value(i);
  ...
  Standard_Integer ishift = (Standard_Integer)( ish <0 ? ish - 0.5 : ish + 0.5 ).getValue();
  ...
  for (Standard_Integer iedge = 1; iedge <= nbe; iedge++) {
	...
	if ( shift != 0. ) {
	  ...
	  c2d->Translate ( V );
        ...
        nbIter = (Standard_Integer)( 1 + Abs ( umax + shift - x ) / myUPeriod ).getValue();
	...
	if ( shift != 0. ) {
	  ...
	  c2d->Translate ( V );
        ...
        nbIter = (Standard_Integer)( 1 + Abs ( umax + shift - y ) / myVPeriod ).getValue();
