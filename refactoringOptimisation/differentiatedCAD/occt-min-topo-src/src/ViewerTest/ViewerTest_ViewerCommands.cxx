                                         ...
                                         Standard_Boolean theIsShift)
{
  if (theToPick)
  {
    ...
    Draw::Set (theArgVec[1], X.getValue());
    Draw::Set (theArgVec[2], Y.getValue());
    Draw::Set (theArgVec[3], Z.getValue());
  }
...
void ProcessZClipMotion()
{
  ...
  if ( Abs(X_Motion - X_ButtonPress) > 2 ) {
    ...
    Standard_Real aDx = a3DView->Convert(X_Motion - X_ButtonPress);
...
static int VSetGradientBg(Draw_Interpretor& di, Standard_Integer argc, const char** argv)
{
  ...
  if (argc == 8)
  {
    ...
    Quantity_Color aColor1(R1.getValue(),G1.getValue(),B1.getValue(),Quantity_TOC_RGB);

    Standard_Real R2 = Draw::Atof(argv[4])/255.;
    Standard_Real G2 = Draw::Atof(argv[5])/255.;
    Standard_Real B2 = Draw::Atof(argv[6])/255.;

    Quantity_Color aColor2(R2.getValue(),G2.getValue(),B2.getValue(),Quantity_TOC_RGB);
...
static int VSetColorBg(Draw_Interpretor& di, Standard_Integer argc, const char** argv)
{
  ...
  if (argc == 4)
  {
    ...
    Quantity_Color aColor(R.getValue(),G.getValue(),B.getValue(),Quantity_TOC_RGB);
...
static int VSetDefaultBg (Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec)
{
  ...
  if (theArgNb == 4)
  {
    ...
    ViewerTest_DefaultBackground.FlatColor.SetValues (R.getValue(), G.getValue(), B.getValue(), Quantity_TOC_RGB);
  }
  else
  {
    ...
    ViewerTest_DefaultBackground.GradientColor1.SetValues (R1.getValue(), G1.getValue(), B1.getValue(), Quantity_TOC_RGB);
    ...
    ViewerTest_DefaultBackground.GradientColor2.SetValues (R2.getValue(), G2.getValue(), B2.getValue(), Quantity_TOC_RGB);
  }
                        ...
                        const char**      theArgVec)
{
  ...
  if (theArgNb <= 2)
  {
    theDI << "Color scale parameters for '"<< theArgVec[1] << "':\n"
          << "Min range: " << aMinRange.getValue() << "\n"
          << "Max range: " << aMaxRange.getValue() << "\n"
  ...
  for (Standard_Integer anArgIter = 2; anArgIter < theArgNb; ++anArgIter)
  {
    ...
    else if (aFlag == "-color")
    {
      ...
      aCS->SetColor     (Quantity_Color (aRedValue.getValue(), aGreenValue.getValue(), aBlueValue.getValue(), Quantity_TOC_RGB), anIndex);
      aCS->SetColorType (Aspect_TOCSD_USER);
      anArgIter += 4;
    }
    else if (aFlag == "-label")
    ...
    else if (aFlag == "-colors")
    {
      ...
      while (aColorIter < theArgNb)
      {
        ...
        else
        {
          ...
          aSeq.Append (Quantity_Color (aRedValue.getValue(), aGreenValue.getValue(), aBlueValue.getValue(), Quantity_TOC_RGB));
          aColorIter += 3;
          anArgIter += 3;
        }
      }
                  ...
                  const char**      theArgVec)
{
  ...
  Standard_Real anOriginX, anOriginY;
  Standard_Real aRotAngle;
                     ...
                     const char**      theArgVec)
{
  ...
  if (aCoord.Length() == 1)
  {
    switch (aMode)
    {
      ...
      case Window : theDI << "Window Vp: " << aView->Convert ((Standard_Real) aCoord (1));  return 0;
      default:
  ...
  if (aCoord.Length() == 2)
  {
    switch (aMode)
    {
      ...
      case Window :
        aView->Convert ((Standard_Real) aCoord (1), (Standard_Real) aCoord (2), aXYp[0], aXYp[1]);
                 ...
                 const char**      theArgVec)
{
  ...
  const double aFpsAver = double(aFramesNb) / aTime;
  const double aCpuAver = aCpu / double(aFramesNb);
...
static int VDiffImage (Draw_Interpretor& theDI, Standard_Integer theArgNb, const char** theArgVec)
{
  ...
  aComparer.SetColorTolerance (aTolColor.getValue());
...
static int VViewParams (Draw_Interpretor& theDi, Standard_Integer theArgsNb, const char** theArgVec)
{
  ...
  if (theArgsNb == 1)
  {
    ...
    Standard_Real anAISViewScale = anAISView->Scale();
    ...
    theDi << "Scale of current view: " << anAISViewScale.getValue() << "\n";
    theDi << "Proj on X : " << anAISViewProjX.getValue() << "; on Y: " << anAISViewProjY.getValue() << "; on Z: " << anAISViewProjZ.getValue() << "\n";
    theDi << "Up on X : " << anAISViewUpX.getValue() << "; on Y: " << anAISViewUpY.getValue() << "; on Z: " << anAISViewUpZ.getValue() << "\n";
    theDi << "At on X : " << anAISViewAtX.getValue() << "; on Y: " << anAISViewAtY.getValue() << "; on Z: " << anAISViewAtZ.getValue() << "\n";
    theDi << "Eye on X : " << anAISViewEyeX.getValue() << "; on Y: " << anAISViewEyeY.getValue() << "; on Z: " << anAISViewEyeZ.getValue() << "\n";
    return 0;
  }
  ...
  if (aMapOfKeysByValues.Find ("SCALE", aValues))
  {
    if (aValues->IsEmpty())
    {
      theDi << "Scale: " << anAISView->Scale().getValue() << "\n";
    }
    else
    {
  ...
  if (aMapOfKeysByValues.Find ("SIZE", aValues))
  {
    if (aValues->IsEmpty())
    {
      ...
      theDi << "Size X: " << aSizeX.getValue() << " Y: " << aSizeY.getValue() << "\n";
    }
    else
    {
  ...
  if (aMapOfKeysByValues.Find ("EYE", aValues))
  {
    if (aValues->IsEmpty())
    {
      ...
      theDi << "Eye X: " << anEyeX.getValue() << " Y: " << anEyeY.getValue() << " Z: " << anEyeZ.getValue() << "\n";
    }
    else
    {
  ...
  if (aMapOfKeysByValues.Find ("AT", aValues))
  {
    if (aValues->IsEmpty())
    {
      ...
      theDi << "At X: " << anAtX.getValue() << " Y: " << anAtY.getValue() << " Z: " << anAtZ.getValue() << "\n";
    }
    else
    {
  ...
  if (aMapOfKeysByValues.Find ("PROJ", aValues))
  {
    if (aValues->IsEmpty())
    {
      ...
      theDi << "Proj X: " << aProjX.getValue() << " Y: " << aProjY.getValue() << " Z: " << aProjZ.getValue() << "\n";
    }
    else
    {
  ...
  if (aMapOfKeysByValues.Find ("UP", aValues))
  {
    if (aValues->IsEmpty())
    {
      ...
      theDi << "Up X: " << anUpX.getValue() << " Y: " << anUpY.getValue() << " Z: " << anUpZ.getValue() << "\n";
    }
    else
    {
                                ...
                                const char ** argv)
{
  ...
  if(argc==1)
  {
    ...
    di << "ZClippingMode = " << aZClippingModeString.ToCString() << "\n"
      << "ZClipping depth = " << aDepth.getValue() << "\n"
      << "ZClipping width = " << aWidth.getValue() << "\n";
  }
  else
  {
    if(argc !=3)
                                ...
                                const char ** argv)
{
  ...
  if (aSize <= 0.)
  {
    di<<"Bad Size value  : " << aSize.getValue() << "\n";
    return 1;
  }
  ...
  if (aCommand == "change")
  {
    ...
    else if (aChangeArg == "capping") // change capping aspects
    {
      ...
      else if (aCappingArg == "color") // color aspect for capping
      {
        ...
        Quantity_Color aColor (aRed.getValue(), aGrn.getValue(), aBlu.getValue(), Quantity_TOC_RGB);
                      ...
                      const char**      theArgVec)
{
  ...
  if (theArgsNb < 2)
  {
    ...
    theDi << "AngularDeflection:  " << (180.0 * aDefParams->HLRAngle() / M_PI).getValue() << "\n";
                   ...
                   const char**      theArgVec)
{
  ...
  if (theArgsNb < 2)
  {
    ...
    for (aView->InitActiveLights(); aView->MoreActiveLights(); aView->NextActiveLights(), ++aLightId)
    {
      ...
      switch (aLight->Type())
      {
        ...
        {
          theDi << "  Type:       Ambient\n";
          theDi << "  Intensity:  " << aLight->Intensity().getValue() << "\n";
          break;
        }
        case V3d_DIRECTIONAL:
        {
          ...
          theDi << "  Smoothness: " << aLight->Smoothness().getValue() << "\n";
          if (!aLightDir.IsNull())
          {
            aLightDir->Position  (anXYZ[0], anXYZ[1], anXYZ[2]);
            theDi << "  Position:   " << anXYZ[0].getValue() << ", " << anXYZ[1].getValue() << ", " << anXYZ[2].getValue() << "\n";
            aLightDir->Direction (anXYZ[0], anXYZ[1], anXYZ[2]);
            theDi << "  Direction:  " << anXYZ[0].getValue() << ", " << anXYZ[1].getValue() << ", " << anXYZ[2].getValue() << "\n";
          }
          break;
        }
        case V3d_POSITIONAL:
        {
          ...
          if (!aLightPos.IsNull())
          {
            ...
            theDi << "  Atten.:     " << anAtten[0].getValue() << " " << anAtten[1].getValue() << "\n";
          }
          break;
        }
        case V3d_SPOT:
        {
          ...
          if (!aLightSpot.IsNull())
          {
            ...
            theDi << "  Atten.:     " << anAtten[0].getValue() << " " << anAtten[1].getValue() << "\n";
            theDi << "  Angle:      " << (aLightSpot->Angle() * 180.0 / M_PI).getValue() << "\n";
            theDi << "  Exponent:   " << aLightSpot->Concentration().getValue() << "\n";
          }
          break;
        }
        default:
  ...
  for (Standard_Integer anArgIt = 1; anArgIt < theArgsNb; ++anArgIt)
  {
          ...
          || anArgCase.IsEqual ("SMOOTHNESS"))
    {
      ...
      {
        Standard_Real tmp;
        Standard_ShortReal aSmoothnessRatio = static_cast<Standard_ShortReal> (tmp = aSmoothness / aLightCurr->Smoothness());
