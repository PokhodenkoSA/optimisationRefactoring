                                         ...
                                         Quantity_Color&   theColor)
{
  ...
  else if (theArgNb >= 3)
  {
    ...
    theColor.SetValues (anRgb.x().getValue(), anRgb.y().getValue(), anRgb.z().getValue(), Quantity_TOC_RGB);
    return 3;
  }
...
static int VSelPrecision(Draw_Interpretor& di, Standard_Integer argc, const char** argv)
{
  ...
  if( argc == 1 )
  {
    ...
    di << "Pixel tolerance : " << aPixelTolerance.getValue() << "\n";
  }
  else if (argc == 2)
  {
  ...
  else if (aCmdName == "vsetcolor")
  {
    ...
    else if (aNames.Length() >= 3)
    {
      ...
      if (isOk)
      {
        ...
        aChangeSet->Color.SetValues (anRgb.x().getValue(), anRgb.y().getValue(), anRgb.z().getValue(), Quantity_TOC_RGB);
        aNames.Remove (aNames.Length());
        aNames.Remove (aNames.Length());
        aNames.Remove (aNames.Length());
      }
          ...
          || anArg == "-setfbcolor")
    {
      ...
      switch (aNbComps)
      {
        ...
        case 3:
        {
          ...
          aChangeSet->FreeBoundaryColor.SetValues (anRgb.x().getValue(), anRgb.y().getValue(), anRgb.z().getValue(), Quantity_TOC_RGB);
          break;
        }
        default:
        {
  ...
  if (toPrintEntities)
  {
    ...
    for (aSelector->InitDetected(); aSelector->MoreDetected(); aSelector->NextDetected())
    {
      ...
      Sprintf (anInfoStr,
               ...
               aResult.Depth().getValue(),
               aResult.DistToGeomCenter().getValue(),
               aDetectedPnt.X().getValue(), aDetectedPnt.Y().getValue(), aDetectedPnt.Z().getValue());
      theDI << "  " << aName
            << anInfoStr
