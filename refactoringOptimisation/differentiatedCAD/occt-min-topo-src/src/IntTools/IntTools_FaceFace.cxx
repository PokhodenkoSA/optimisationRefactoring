    ...
    if (!myApprox) {
      ...
      for (i=1; i<=aNbParts; ++i) {
        ...
        if(bAvoidLineConstructor) {
          ...
          myLConstruct.Part(i, fprm, lprm);
          ifprm=(Standard_Integer)fprm.getValue();
          ilprm=(Standard_Integer)lprm.getValue();
        }
        //
        if(myApprox1) {
      ...
      if(i != k) {
           ...
           Abs(UV[2] - UV[6]) + Abs(UV[3] - UV[7]) < 1.0e-16 * aMax )
        {
          ...
          for(v = 1; v <= aTmpWLine->NbVertex(); v++) {
            ...
            Standard_Integer avertexindex = (Standard_Integer)aVertex.ParameterOnLine().getValue();
      ...
      for ( i = 1; i <= anExtrema.NbExt(); i++ ) {
        ...
        if ( bFoundResult ) {
          ...
          for ( Standard_Integer k1 = 0; k1 < 2; k1++ ) {
            ...
            Standard_Real aShift = ( k1 == 0 ) ? twoPI : (tmp = -twoPI);
                               ...
                               const TopoDS_Face& aFace) 
{
  ...
  if (aSurf->IsUPeriodic()) {
    ...
    Standard_Integer nshift = (Standard_Integer) ((u-umin)/aPer).getValue();
  ...
  if (aSurf->IsVPeriodic()) {
    ...
    Standard_Integer nshift = (Standard_Integer) ((v-vmin)/aPer).getValue();
