                                        ...
                                        const Standard_Integer level)  const
{
  ...
  S << endl << "Knots : ";
  IGESData_DumpValsAD(S ,level,-ent->Degree(), upind+1,ent->Knot);
  S << endl << "Weights : ";
  IGESData_DumpValsAD(S ,level,0, upind,ent->Weight);
  S << endl << "Control Points (Poles) : ";
  IGESData_DumpListXYZL(S ,level,0, upind, ent->Pole, ent->Location());
  S << endl << "Starting Parameter Value : " << ent->UMin().getValue();
  S << "  " << "Ending Parameter Value : " << ent->UMax().getValue() << endl;
  S << "Unit Normal : ";
