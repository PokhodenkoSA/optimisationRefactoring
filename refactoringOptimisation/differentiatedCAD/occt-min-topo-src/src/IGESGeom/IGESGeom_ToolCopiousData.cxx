                                       ...
                                       const Standard_Integer level)  const
{
  ...
  if (dtype == 1) {
    S << "(Points 2D)  ";
    S << "ZPlane = " << ent->ZPlane().getValue() << endl;
  }
  else if (dtype == 2)
    S << "(Points 3D)" << endl;
  else if (dtype == 3)
  ...
  if (level > 4) {
    for (i = 1; i <= nbPnts; i ++) {
      ...
      if (dtype == 1)
	S << "[" << i << "] (" << T.X().getValue() << "," << T.Y().getValue() << ")";
      else
	S << "[" << i << "] (" << T.X().getValue() << "," << T.Y().getValue() << "," << T.Z().getValue() << ")";

      if (yatr) {
	loca.Transforms(T);
	S << " Transformed ("<< T.X().getValue() << "," << T.Y().getValue() << "," << T.Z().getValue() << ")";
      }

      if (dtype == 3) {
//	gp_XYZ P = ent->Vector(i).XYZ();
	S << "\n   Vector (" << T.X().getValue() << "," << T.Y().getValue() << "," << T.Z().getValue() << ")";
	if (yatr) {
	  locv.Transforms(T);
	  S << " Transformed ("<< T.X().getValue() << "," << T.Y().getValue() << "," << T.Z().getValue() << ")";
	}
