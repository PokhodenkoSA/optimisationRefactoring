                                          ...
                                          const Standard_Integer level)  const
{
  ...
  S << "Starting Parameter for U direction : " << ent->UMin().getValue() << endl;
  S << "Ending   Parameter for U direction : " << ent->UMax().getValue() << endl;
  S << "Starting Parameter for V direction : " << ent->VMin().getValue() << endl;
  S << "Ending   Parameter for V direction : " << ent->VMax().getValue() << endl;

  S << " Knots in  U : ";
  IGESData_DumpValsAD(S ,level,-degU, indU+1,ent->KnotU);
  S << endl;
  S << " Knots in  V : ";
  IGESData_DumpValsAD(S ,level,-degV, indV+1,ent->KnotV);
  S << endl;
