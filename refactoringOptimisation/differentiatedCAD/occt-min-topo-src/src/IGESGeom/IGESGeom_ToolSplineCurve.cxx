   ...
   const Handle(Message_Messenger)& S, const Standard_Integer level)  const
{
  ...
  IGESData_DumpValsAD(S ,level,1, nbSegments+1,ent->BreakPoint);
  if (level <= 4) {
...
//  Standard_Real AX,BX,CX,DX, AY,BY,CY,DY, AZ,BZ,CZ,DZ;
    ...
    S << " A ...	"<<AX.getValue()<<"	"<<AY.getValue()<<"	"<<AZ.getValue()<<endl;
    S << " B ...	"<<BX.getValue()<<"	"<<BY.getValue()<<"	"<<BZ.getValue()<<endl;
    S << " C ...	"<<CX.getValue()<<"	"<<CY.getValue()<<"	"<<CZ.getValue()<<endl;
    S << " D ...	"<<DX.getValue()<<"	"<<DY.getValue()<<"	"<<DZ.getValue()<<endl;
  }
  ent->XValues(AX,BX,CX,DX);
  ent->YValues(AY,BY,CY,DY);
  ent->ZValues(AZ,BZ,CZ,DZ);
  S << "Terminate Point :	    X		   Y		Z"<<endl;
  S << " Value        	"<<AX.getValue()<<"	"<<AY.getValue()<<"	"<<AZ.getValue()<<endl;
  S << " 1st Derivative	"<<BX.getValue()<<"	"<<BY.getValue()<<"	"<<BZ.getValue()<<endl;
  S << " 2nd Der./2!   	"<<CX.getValue()<<"	"<<CY.getValue()<<"	"<<CZ.getValue()<<endl;
  S << " 3rd Der./3!   	"<<DX.getValue()<<"	"<<DY.getValue()<<"	"<<DZ.getValue()<<endl;
}
