...
float MyHue, MyLight, MySaturation;

	Quantity_Color::rgbhls
			(MyRed, MyGreen, MyBlue, MyHue, MyLight, MySaturation);

	MySaturation	+= MySaturation * float (ADelta) / (float ) 100.0;
...
float MyHue, MyLight, MySaturation;
	...
	MyLight		+= MyLight * float (ADelta) / (float ) 100.0;
...
double Quantity_Color::Distance (const Quantity_Color& AColor) const {
...
double R2, G2, B2;
	...
	R2	= double (AColor.MyRed);
	G2	= double (AColor.MyGreen);
	B2	= double (AColor.MyBlue);
...
double Quantity_Color::SquareDistance (const Quantity_Color& AColor) const {
...
double NewDist = 4.0;			// Distance courante.
double Dist = 4.0;			// Distance la plus courte.
...
void Quantity_Color::rgbhls (const float R, const float G, const float B, float& H, float& L, float& S) {

	call_rgbhls (R, G, B, H, L, S);

}

void Quantity_Color::hlsrgb (const float H, const float L, const float S, float& R, float& G, float& B) {
...
void Quantity_Color::ValuesOf (const Quantity_NameOfColor AName, const Quantity_TypeOfColor AType, float& R1, float& R2, float& R3) {
