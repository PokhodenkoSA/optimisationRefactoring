                             ...
                             BRepMesh::SequenceOfReal&   theResult)
{
  ...
  if( theResult.Length() == 2 )
  {
    ...
    Standard_Integer nbint = (Standard_Integer)((dist / theFilterDist) + 0.5).getValue();
  ...
  BRepMesh::ListOfVertex& theNewVertices)
{
  ...
  for (Standard_Integer i = 0; i < 2; ++i)
  {
    ...
    aDd[i] = aDiff / ((Standard_Integer)(aDiff / aDd[i]).getValue() + 1);
  ...
  BRepMesh::ListOfVertex& theNewVertices)
{
  ...
  const Standard_Integer nbU = (Standard_Integer)(su / Du).getValue();
  Du = su / (nbU + 1);

  // Calculate parameters for iteration in V direction
  const Standard_Real sv = vmax - vmin;
  Standard_Integer nbV = (Standard_Integer)(nbU*sv / (su*aRadius)).getValue();
  nbV = Min(nbV, 100 * nbU);
  ...
  BRepMesh::ListOfVertex& theNewVertices)
{
  ...
  Standard_Integer nbU = (Standard_Integer)((umax - umin) / Du).getValue();
  Standard_Integer nbV = (Standard_Integer)(nbU*(vmax - vmin) / ((umax - umin)*aRadius)).getValue();
  Du = (umax - umin) / (nbU + 1);
  Dv = (vmax - vmin) / (nbV + 1);
  ...
  BRepMesh::ListOfVertex& theNewVertices)
{
  ...
  Standard_Real Du;
  Standard_Integer nbV = Max((Standard_Integer)((vmax - vmin) / Dv).getValue(), 2);
  Dv = (vmax - vmin) / (nbV + 1);
  ...
  Standard_Integer nbU = Max((Standard_Integer)((umax - umin) / Du).getValue(), 2);
  nbU = Max(nbU, (int)(nbV*(umax - umin)*R / ((vmax - vmin)*r) / 5.).getValue());
  Du = (umax - umin) / (nbU + 1);
