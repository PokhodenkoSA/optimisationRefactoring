...
void Bnd_BoundSortBox::SortBoxes() 
{
  ...
  for (labox=i0; labox<=i1; labox++) {
    if (!taBox(labox).IsVoid()) {
      ...
      if(xmin>Xmin) firstcaseX=(Standard_Integer )((xmin-Xmin)*deltaX).getValue()-1; else  firstcaseX=1;
      if(ymin>Ymin) firstcaseY=(Standard_Integer )((ymin-Ymin)*deltaY).getValue()-1; else  firstcaseY=1;
      if(zmin>Zmin) firstcaseZ=(Standard_Integer )((zmin-Zmin)*deltaZ).getValue()-1; else  firstcaseZ=1;
      if(xmax<_Xmax) lastcaseX=(Standard_Integer )((xmax-Xmin)*deltaX).getValue()+1; else  lastcaseX=discrX;
      if(ymax<_Ymax) lastcaseY=(Standard_Integer )((ymax-Ymin)*deltaY).getValue()+1; else  lastcaseY=discrY;
      if(zmax<_Zmax) lastcaseZ=(Standard_Integer )((zmax-Zmin)*deltaZ).getValue()+1; else  lastcaseZ=discrZ;
			   ...
			   const Standard_Integer boxIndex)
{
  ...
  if (!theBox.IsVoid()) {
    ...
    if(xmin>Xmin) firstGapX=(Standard_Integer )((xmin-Xmin)*deltaX).getValue()-1; else  firstGapX=1;
    if(ymin>Ymin) firstGapY=(Standard_Integer )((ymin-Ymin)*deltaY).getValue()-1; else  firstGapY=1;
    if(zmin>Zmin) firstGapZ=(Standard_Integer ) ((zmin-Zmin)*deltaZ).getValue()-1; else  firstGapZ=1;
    if(xmax<_Xmax) lastGapX=(Standard_Integer )((xmax-Xmin)*deltaX).getValue()+1; else  lastGapX=discrX;
    if(ymax<_Ymax) lastGapY=(Standard_Integer )((ymax-Ymin)*deltaY).getValue()+1; else  lastGapY=discrY;
    if(zmax<_Zmax) lastGapZ=(Standard_Integer )((zmax-Zmin)*deltaZ).getValue()+1; else  lastGapZ=discrZ;
...

{
 Standard_Integer lacase ;
  ...
  if(xmin>Xmin) i0=(Standard_Integer )((xmin-Xmin)*deltaX).getValue()-1; else  i0=1;
  if(ymin>Ymin) j0=(Standard_Integer )((ymin-Ymin)*deltaY).getValue()-1; else  j0=1;
  if(zmin>Zmin) k0=(Standard_Integer )((zmin-Zmin)*deltaZ).getValue()-1; else  k0=1;
  if(xmax<_Xmax) i1=(Standard_Integer )((xmax-Xmin)*deltaX).getValue()+1; else  i1=discrX;
  if(ymax<_Ymax) j1=(Standard_Integer )((ymax-Ymin)*deltaY).getValue()+1; else  j1=discrY;
  if(zmax<_Zmax) k1=(Standard_Integer )((zmax-Zmin)*deltaZ).getValue()+1; else  k1=discrZ;
