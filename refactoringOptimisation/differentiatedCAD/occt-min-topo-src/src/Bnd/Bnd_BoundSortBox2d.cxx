				    ...
				    const Handle(Bnd_HArray1OfBox2d)& SetOfBox)
{
  ...
  if (middleX < Epsilon(100.)) {
    ...
    discrX=(Standard_Integer)((Xmax-Xmin)/middleX).getValue();
    deltaX=middleX;
    discrX++;
  }
  ...
  if (middleY < Epsilon(100.)) {
    ...
    discrY=(Standard_Integer)((Ymax-Ymin)/middleY + 0.1).getValue();
    deltaY=middleY;
    discrY++;
  }
...
void  Bnd_BoundSortBox2d::SortBoxes()
{
  ...
  for (labox=taBox.Lower(); labox<=taBox.Upper(); labox++) {
    if (!taBox(labox).IsVoid()) {
      ...
      if (discrX>1) {
	firstcase=(Standard_Integer ) Max(1.0, (xmin-Xmin)/deltaX).getValue();
	lastcase=(Standard_Integer ) Min((Standard_Real)discrX, ((xmax-Xmin)/deltaX)+1).getValue();
	for (lacase=firstcase; lacase<=lastcase; lacase++) {
      ...
      if (discrY >1) {
	firstcase=(Standard_Integer ) Max(1.0, (ymin-Ymin)/deltaY).getValue();
	lastcase=(Standard_Integer ) Min((Standard_Real)discrY, ((ymax-Ymin)/deltaY)+1).getValue();
	for (lacase=firstcase; lacase<=lastcase; lacase++) {
			      ...
			      const Standard_Integer boxIndex)
{
  ...
  if (!theBox.IsVoid()) {
    ...
    if (discrX>1) {
      firstGap=(Standard_Integer ) Max(1.0, ((xmin-Xmin)/deltaX)+1).getValue();
      lastGap=(Standard_Integer ) Min((Standard_Real)discrX, ((xmax-Xmin)/deltaX)+1).getValue();
    ...
    if (discrY >1) {
      firstGap=(Standard_Integer ) Max(1.0, ((ymin-Ymin)/deltaY)+1).getValue();
      lastGap=(Standard_Integer ) Min((Standard_Real)discrY, ((ymax-Ymin)/deltaY)+1).getValue();
      for (theGap=firstGap; theGap<=lastGap; theGap++) {
