   ...
   const Handle(Message_Messenger)& S, const Standard_Integer level)  const
{
  ...
  S << "Total Number Of Possible Instance Locations : " << ent->NbLocations()
    << endl;
  S << "Imaginary Circle. Radius : " << ent->CircleRadius().getValue() << "  Center : ";
  IGESData_DumpXYZL(S, level, ent->CenterPoint(), ent->Location());  S << endl;
  S << "Start Angle (in radians) : " << ent->StartAngle().getValue() << "  ";
  S << "Delta Angle (in radians) : " << ent->DeltaAngle().getValue() << endl;
  S << "Do-Dont Flag : ";
