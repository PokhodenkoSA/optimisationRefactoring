			       ...
			       const Standard_Integer ISize)
{
  ...
  gp_Pnt2d p2 = p1;
  gp_Circ2d C;
  Standard_Real Size = ((double) ISize) / Zoom();
			       ...
			       const Standard_Real R)
{
  ...
  case Draw_Circle :
    {
      Standard_Integer I = (Standard_Integer ) R.getValue();
      if(!I) return;
      DrawMarker(pt, S, I);
      break;
    }
			...
			const Standard_Boolean ModifyWithZoom)
{
  ...
  Standard_Integer n = (Standard_Integer )( (A2 - A1) / angle).getValue();
  if (n > MAXPNT) {
    angle = (A2 - A1) / MAXPNT;
    n = MAXPNT;
  }
  ...
  gp_Circ Cloc(C);
  if(!ModifyWithZoom) {
    Standard_Integer ISize = (Standard_Integer )( Cloc.Radius() / Zoom()).getValue();
...
void Draw_Display::Draw(const gp_Circ2d& C, const Standard_Real A1, Standard_Real A3,
			const Standard_Boolean ModifyWithZoom)
{
  Standard_Real A2 = A3;
  while (A2 < A1) A2 += 2*M_PI;
